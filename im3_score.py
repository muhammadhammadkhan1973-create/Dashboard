...
def score_ticker(ticker):
    ...
    iv_comp = avg(ivs)
    # Triangulated central intrinsic value = MEDIAN of the valid per-share methods. The median
    # is robust to any single method running away (e.g. a generous Graham-revised on a hyper-
    # grower) — this is the master's "triangulation" read. The headline MoS is measured against
    # this central estimate, not DCF-EPS alone. Non-positive / missing methods are excluded.
    _tri = sorted(ivs)
    if _tri:
        _m = len(_tri)
        iv_central = _tri[_m//2] if _m % 2 else (_tri[_m//2 - 1] + _tri[_m//2]) / 2.0
    else:
        iv_central = None
    mos_pct = _mos(iv_central if iv_central else iv_eps)
