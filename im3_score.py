...
def score_bank_us(sc):
    ...
    naltot = num('naltot'); ore = num('ore') or 0.0
    if naltot is not None and naltot <= 0:        # no noncurrent loans -> fully covered / no credit stress
        P['reserve_cov'] = 2; P['texas'] = 2
    else:
        cov = (100 * allow / naltot) if (allow is not None and naltot) else None
        P['reserve_cov'] = b(cov, *C['reserve_cov'])
        texas = (100 * (naltot + ore) / (tce + allow)) if (tce is not None and allow is not None and (tce + allow)) else None
