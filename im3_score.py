...
_SA_US_ROWMAP = {   # our history key -> stockanalysis standardized row label
...
def _sa_us_complete(ticker, h):
    """Fill empty balance-sheet series in h from the stockanalysis US page. Returns count filled."""
    needed = [k for k, lbl in _SA_US_ROWMAP.items() if not (h.get(k) and any(x is not None for x in h[k]))]
    if not needed:
        return 0
    try:
        bare = ticker.split(':')[-1].lower()
        tbl = _sa_fetch_statement(_SA_US_BAL.format(t=bare))
        if not tbl:
            return 0
    except Exception:
        return 0
    ...
    for k in needed:
        ...
        if series and any(x is not None for x in series):
            ...
            filled += 1
    if filled:
        h.setdefault('sa_completed', []).extend(needed[:filled])
    return filled
...
def score_ticker(ticker):
    ...
    try:
        ...
        if dy is not None: _mv('div_yield', dy*100, '%')
        if ev_eb is not None: _mv('ev_ebitda', ev_eb, 'x')
        if nsy is not None: _mv('val_shareholders', nsy*100, '%')
