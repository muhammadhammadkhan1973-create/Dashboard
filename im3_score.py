...
def _sa_us_complete(ticker, h):
    ...
    for url, rowmap in ((_SA_US_BAL, _SA_US_ROWMAP), (_SA_US_INC, _SA_US_INC_ROWMAP), (_SA_US_CF, _SA_US_CF_ROWMAP)):
        ...
        for k in needed:
            ...
            if not series:
                continue
            # v2.28.1 ROOT-CAUSE FIX (the 52-name 'int + str' crash, self-diagnosed by the new
            # data['im3_errors'] on its FIRST run): _sa_fetch_statement returns RAW STRING cells
            # ('43.69', '(0.56)', '-'). v2.28.0 assigned them UNPARSED into numeric history series,
            # so any downstream arithmetic raised TypeError and score_ticker returned None -- this
            # was ALSO the chronic pre-2.28 failure (MU/NVDA/DOCN/CDE/RSI/IDR) via the v2.27
            # balance-only completion. Every cell is now parsed with _sa_num and money rows are
            # scaled x1e6 to the SEC series' absolute units (SA serves millions), exactly like the
            # proven PSX put() path. EPS is per-share and stays UNSCALED; int_exp/div_paid/buyback
            # become magnitudes. All-None rows are skipped, never assigned.
            _money = k not in ('eps_s',)
            parsed = [(None if (n := _sa_num(v)) is None else (n * 1e6 if _money else n)) for v in series]
            if k in ('int_exp', 'div_paid', 'buyback'):
                parsed = [abs(x) if x is not None else None for x in parsed]
