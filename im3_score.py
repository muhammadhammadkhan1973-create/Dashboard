...
def score_ticker(ticker):
    ...
    iv_comp = avg(ivs)
    # Triangulated central intrinsic value = MEDIAN of the defensible earnings/cash/asset methods:
    # DCF·EPS (Graham-revised, earnings power), DCF·FCF (2-stage on free cash flow), Graham number
    # (asset/earnings floor), and Peter-Lynch (PEG=1 growth value). The total-CASH 2-stage DCF is
    # EXCLUDED from the anchor — capitalising cash-per-share as a growing perpetuity is noisy — but
    # it stays in the expand. Median is robust to any single method; non-positive ones are dropped.
    _core = sorted(v for v in [iv_eps, iv_dcf_fcf, iv_gr, iv_pl] if v and v > 0)
    if _core:
        _m = len(_core)
    ...
    if iv_central and price and (iv_central > price * 10 or iv_central < price / 10):
        iv_central = None
    # MoS vs the central IV, CLAMPED to [-100%, +100%]. The raw (IV-price)/IV is unbounded on the
    # downside (a name priced at 7x a small IV would read -600%), so the overvalued tail is floored
    # at -100% ("fully overvalued / no margin"); the cheap side keeps the true Graham margin (0..+100).
    _mraw = _mos(iv_central) if iv_central else None
    mos_pct = max(-100.0, min(100.0, _mraw)) if _mraw is not None else None
