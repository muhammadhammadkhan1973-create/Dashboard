...
def build_etf_recommendations(country_rs, sector_booming, global_theme):
    ...
    try:
        ...
        log(f"  [ETF Recommendations] {len(recs)} resolved: " +
            ', '.join(f"{r['door']}:{r['signal']}→{r['fund']['name'][:30]}" for r in recs))
    except Exception as _e:
...
def fetch_country_rs():
    ...
    try:
        # Read from current-run data dict (metals already fetched); key is 'dxy' not 'dxy_px'
        _metals_now = (data.get('macros') or {}).get('metals') or {}
        _dxy_trend  = _metals_now.get('dxy_ma_trend') or safe_get(EXISTING, 'macros', 'metals', 'dxy_ma_trend') or 'unknown'
        _dxy_sma200 = _metals_now.get('dxy_sma200')  or safe_get(EXISTING, 'macros', 'metals', 'dxy_sma200')
        _dxy_px     = _metals_now.get('dxy')         or safe_get(EXISTING, 'macros', 'metals', 'dxy')
