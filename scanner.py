...
_PSX_NAME_TICKER = [
    ...
    ('pakistan petroleum','PPL'), ('mari energies','MARI'), ('mari petroleum','MARI'),
    ('pakistan oilfields','POL'),
    ('pakistan state oil','PSO'), ('attock refinery','ATRL'), ('national refinery','NRL'),
    ...
    ('faysal bank','FABL'), ('bank of punjab','BOP'), ('js bank','JSBL'),
...
def fetch_fmr_fund_ownership():
    ...
    for slug, amc in _FMR_SOURCES:
        ...
        try:
            ...
            md = re.search(r'as at ([A-Z][a-z]+\.? \d{1,2},? \d{4})', text)
            asof = md.group(1) if md else '?'
            for tk, w, _ in hold:
                agg[tk].append(w)
            for raw, w in unm:
...
def main():
    ...
    # Wave P Phase-1: AMC fund-ownership overlay (DISPLAY ONLY; not a scored stream -> respects freeze)
    try:
        _fmr_by_ticker, _fmr_meta = fetch_fmr_fund_ownership()
        if _fmr_by_ticker:
