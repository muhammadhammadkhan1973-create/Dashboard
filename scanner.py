...
def fetch_countryeconomy(data, existing):
    ...
    for key, slug in _CE_SLUG.items():
        ...
        try:
            ...
            if d is not None:
                rec['debt_gdp'], rec['debt_year'] = d, yr
        except Exception as e:
...
M1_GATE_PCT = 61          # the group's strict bar: 61% (75/122) of the deep scorecard
M1_PER_SECTOR = 12        # deep-score the top N keystone big-caps per favored sector (quick-score lane)
M1_MCAP_LANE = 6          # PLUS the top N by MARKET CAP per favored sector regardless of quick-score rank --
...
def build_m1_buylist(data, existing):
    ...
    disc = ((data.get('m2_universe') or {}).get('disciplined')) or []
    ...
    # candidate pool per favored sector = quick-score lane UNION market-cap lane (dedup by ticker).
    ...
    for g in favored:
        ...
        for r in by_score + by_mcap:
            if r['ticker'] not in seen:
                seen.add(r['ticker'])
                pool.append(dict(r, gics=g))
    today = dt.date.today()
    scored, fresh = [], 0
    for r in pool:
        t = r['ticker']
        ...
        if pv:
            try:
                ...
                age = None
        if pv and age is not None and age <= M1_IM3_TTL_DAYS:
            rec.update({k: pv.get(k) for k in ('im3_pct', 'im3_grade', 'valuation', 'price', 'scored_at')})
        elif fresh < M1_IM3_BUDGET:
            fresh += 1
            try:
                im3 = score_im3(t)
            except Exception as e:
                log(f'  [M1 buylist] scorer failed {t}: {e.__class__.__name__} -- not scored (soft)')
                im3 = None
            if im3:
                val = im3.get('valuation')
                rec.update({'im3_pct': im3.get('pct'), 'im3_grade': im3.get('grade'),
                            'valuation': (val.get('label') if isinstance(val, dict) else val),
                            'price': im3.get('price'), 'scored_at': today.isoformat()})
        ...
        scored.append(rec)
    eligible = [r for r in scored if r['im3_pct'] is not None and r['im3_pct'] >= M1_GATE_PCT]
    ...
    data['m1_buylist'] = {
        ...
        'gate_pct': M1_GATE_PCT, 'pool_n': len(pool), 'fresh_scored': fresh,
    ...
    log(f"  [M1 buylist] regime={regime} pool={len(pool)} fresh-scored={fresh} "
        f"deep-scored={data['m1_buylist']['n_scored']} passed-{M1_GATE_PCT}%={len(eligible)} "
