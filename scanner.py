...
M1_PER_SECTOR = 6         # deep-score the top N keystone big-caps per favored sector
M1_IM3_TTL_DAYS = 7       # carry a name's deep score this long before re-fetching
M1_IM3_BUDGET = 30        # max FRESH deep-score fetches per run (bounded runtime)
...
def build_m1_buylist(data, existing):
    ...
    # candidate pool: top keystone big-caps per favored sector
    pool = []
    for g in favored:
        rows = [r for r in disc if _M1_FINE_TO_GICS.get(r.get('sector')) == g]
        rows.sort(key=lambda r: -(r.get('score') or 0))
        pool += [dict(r, gics=g) for r in rows[:M1_PER_SECTOR]]
    today = dt.date.today()
    scored, fresh = [], 0
    for r in pool:
        t = r['ticker']
