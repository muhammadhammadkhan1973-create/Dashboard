...
JUSTETF_HOLDINGS_PROBE = False   # probe done (justETF profile reachable); using the EXISTING holdings plumbing instead
...
def _attach_discovery_holdings(data, existing=None):
    ...
    Priority per fund, cheapest first:
      ...
      3) fetch_etf_holdings(ticker) via stockanalysis for funds with a US-listed sibling (existing fetcher).
    ...
    for d in uncovered:
        ...
        if fetched >= FETCH_BUDGET:
            still += 1
            continue
        rows = fetch_ishares_holdings(iz)                       # pinned iShares (existing plumbing)
        if not rows:
            tkr = (d.get('sym') or '').split(':')[-1].strip()   # LSE:SMH -> SMH
            if tkr:
                rows = fetch_etf_holdings(tkr)                  # stockanalysis (existing plumbing)
        hs = _hstr(rows)
        if hs:
            d['holdings'] = hs
            cache[iz] = hs
    ...
    log(f'  [discovery holdings] {reuse} reuse (xlsx/watchlist) + {fetched} fetched (iShares/stockanalysis, cached) '
        f'/ {len(disc)} discovery funds; {still} still \u2014 (UCITS-only, no US sibling)')
