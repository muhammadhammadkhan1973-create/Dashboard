...
DEFAULT_DATA = {
    ...
    'rate_path': [],
    # ── Phase 0 keys ──
    'world_lei':      {},   # OECD composite leading indicators
    'us_diffusion':   {},   # US 15-indicator diffusion composite
    'country_rs':     [],   # 9-country RS + currency split + hedge flag
    'sector_booming': [],   # Sector booming bands (Booming/Favoured/Neutral/Lagging)
}
...
# ── Phase 0-A: WORLD LEI + US DIFFUSION COMPOSITE ─────────────────────────
# Fetches OECD Composite Leading Indicators for 9 economies plus the US
# 15-indicator diffusion engine rebuilt from already-fetched US macro fields.
# Freeze-safe: pure macro display, no grade/IM3/TCE touched.
# ───────────────────────────────────────────────────────────────────────────
def fetch_world_lei():
    """
    Returns dict: {
...
# Scores the ~15 US macro indicators already in data['macros']['us'] and
# emits a net composite (supportive − deteriorating) + phase label.
# Freeze-safe: reads only already-fetched macro fields.
# ───────────────────────────────────────────────────────────────────────────
def compute_us_diffusion(us_macros):
    """
    Takes the us_macros dict (already fetched) and returns:
    { 'scores': { indicator: score }, 'net': int,
      'supportive': int, 'deteriorating': int,
      'phase': str, 'direction': str }
    Scores: +1 supportive, -1 deteriorating, 0 neutral.
    """
    scores = {}
    ...
    def s(key, good_fn):
        ...
        if v is None:
            return
        try:
            scores[key] = 1 if good_fn(float(v)) else -1
        except Exception:
            pass

    fed  = us_macros.get('fed_rate', 5.5)
    y10  = us_macros.get('us_10y', 4.5)
    y2   = us_macros.get('us_2y', 4.8)

    # 1. Fed funds rate — lower = supportive (≤4% = clear ease)
    s('fed_rate',       lambda v: v <= 4.0)
    # 2. 10-yr yield — moderate = supportive (3–5%)
    s('us_10y',         lambda v: 3.0 <= v <= 5.0)
    # 3. Yield curve (10y-2y) — positive = supportive
    if y10 is not None and y2 is not None:
        scores['yield_curve'] = 1 if (float(y10) - float(y2)) > 0 else -1
    # 4. Core PCE — below 3% = supportive
    s('core_pce',       lambda v: v < 3.0)
    # 5. CPI YoY — below 3.5% = supportive
    s('cpi_yoy',        lambda v: v < 3.5)
    # 6. Unemployment — below 5% = supportive
    s('unemployment',   lambda v: v < 5.0)
    # 7. Consumer sentiment (UMCSI) — above 70 = supportive
    s('umcsi',          lambda v: v > 70)
    # 8. Building permits — above 1.35M = supportive
    s('permits',        lambda v: v > 1.35)
    # 9. Industrial production — positive YoY = supportive (stored as index level; use >100)
    s('industrial_prod',lambda v: v > 100)
    # 10. GDP growth — above 1.5% = supportive
    s('gdp_growth',     lambda v: v > 1.5)
    # 11. HY spread — below 400bps = supportive (credit stress low)
    s('hy_spread',      lambda v: v < 400)
    # 12. Manufacturing employment — above 12.8M = supportive
    s('mfg_emp',        lambda v: v > 12800)
    # 13. WTI — not runaway (under $90) = supportive for demand
    s('wti',            lambda v: v < 90)
    # 14. 10yr-fed spread as proxy for financial conditions
    if y10 is not None and fed is not None:
        scores['fin_conditions'] = 1 if (float(y10) - float(fed)) < 1.5 else -1
    # 15. Brent-WTI spread (< $5 = market functioning)
    bwspr = us_macros.get('brent_wti_spread')
    if bwspr is not None:
        try:
            scores['brent_wti_spread'] = 1 if abs(float(bwspr)) < 5 else -1
        except Exception:
            pass

    supportive    = sum(1 for v in scores.values() if v == 1)
    deteriorating = sum(1 for v in scores.values() if v == -1)
    net           = supportive - deteriorating
    total         = len(scores)

    if net >= 4:    phase = 'Expansion'
    elif net >= 0:  phase = 'Slowdown'
    else:           phase = 'Contraction'

    direction = 'Accelerating' if net > 0 else ('Decelerating' if net < 0 else 'Flat')

    log(f'US diffusion composite: {supportive}/{total} supportive, '
        f'net={net}, phase={phase}')
    return {
        'scores':       scores,
        'net':          net,
...
# ── Phase 0-C: COUNTRY RELATIVE STRENGTH + CURRENCY SPLIT ────────────────
# Pulls the 9-country index levels + FX from public Yahoo Finance quotes.
# Derives trailing 12m USD return, local return, currency contribution,
# 200-DMA trend, and a hedge flag.
# Freeze-safe: pure display, no grade/screen touched.
# ───────────────────────────────────────────────────────────────────────────
def fetch_country_rs():
    """
    Returns list of dicts, one per country, sorted by usd_return desc:
    { code, name, index_ticker, fx_ticker,
      local_return_pct, fx_return_pct, usd_return_pct,
      above_200dma, trend_slope,
      hedge_call, signal_door, as_of }
    Uses Yahoo Finance for index + FX monthly close.
    """
    ...
    COUNTRIES = [
        ...
        ('GBR', 'UK',           '^FTSE',    'GBPUSD=X',  'Country LEI'),
        ('JPN', 'Japan',        '^N225',    'JPYUSD=X',  'Country LEI'),
        ('NZL', 'New Zealand',  '^NZ50',    'NZDUSD=X',  'Country'),
        ('DEU', 'Germany',      '^GDAXI',   'EURUSD=X',  'Country'),
        ('USA', 'USA',          '^GSPC',    None,         'Confirm-only'),
    ]

    def _yf_monthly(ticker, months=14):
        """Fetch ~14 months of monthly closes from Yahoo Finance."""
        import datetime as _dt
        end   = int(_dt.datetime.utcnow().timestamp())
        start = int((_dt.datetime.utcnow() - _dt.timedelta(days=months*31)).timestamp())
        url = (
            f'https://query1.finance.yahoo.com/v8/finance/chart/{ticker}'
            f'?period1={start}&period2={end}&interval=1mo&events=history'
        )
        try:
            ...
            with urllib.request.urlopen(req, timeout=15) as r:
                raw = _json.loads(r.read())
            closes = raw['chart']['result'][0]['indicators']['adjclose'][0]['adjclose']
            timestamps = raw['chart']['result'][0]['timestamp']
            pairs = [(t, c) for t, c in zip(timestamps, closes) if c is not None]
            return pairs
        except Exception as e:
            warn(f'  country_rs YF fetch {ticker}: {e}')
            return []

    results = []
    for code, name, idx_tkr, fx_tkr, door in COUNTRIES:
        try:
            idx_pts = _yf_monthly(idx_tkr)
            if len(idx_pts) < 13:
                warn(f'  country_rs: insufficient data for {code} index')
                continue

            # 12-month local return
            local_12m_ago = idx_pts[-13][1]
            local_now     = idx_pts[-1][1]
            local_ret     = round((local_now / local_12m_ago - 1) * 100, 1)

            # 200-DMA proxy: use 12-month average of monthly closes
            closes_12 = [v for _, v in idx_pts[-12:]]
            dma200    = sum(closes_12) / len(closes_12)
            above_200 = local_now > dma200
            # slope: last 3 months vs prior 3
            if len(closes_12) >= 6:
                recent3 = sum(closes_12[-3:]) / 3
                prior3  = sum(closes_12[-6:-3]) / 3
                slope   = round((recent3 / prior3 - 1) * 100, 2)
            else:
                slope = 0.0

            # Currency return vs USD
            if fx_tkr and code != 'USA':
                _time.sleep(0.3)
                fx_pts = _yf_monthly(fx_tkr)
                if len(fx_pts) >= 13:
                    fx_12m_ago = fx_pts[-13][1]
                    fx_now     = fx_pts[-1][1]
                    fx_ret     = round((fx_now / fx_12m_ago - 1) * 100, 1)
                else:
                    fx_ret = 0.0
                usd_ret = round(local_ret + fx_ret + (local_ret * fx_ret / 100), 1)
            else:
                fx_ret  = 0.0
                usd_ret = local_ret
            ...
            if abs(fx_ret) < 1.0:
                ...
                hedge = 'Hedge (FX drag)'
            elif fx_ret > 3.0 and abs(local_ret) < 5.0:
                hedge = 'Never hedge (FX IS the return)'
            else:
                hedge = 'Lean unhedged'
            ...
            results.append({
                ...
                'local_return_pct': local_ret,
                'fx_return_pct':   fx_ret,
            ...
            warn(f'  country_rs: {code} failed: {e}')
            continue

    results.sort(key=lambda x: x.get('usd_return_pct', 0), reverse=True)
    log(f'Country RS: {len(results)} countries fetched')
    return results


# ── Phase 0-D: SECTOR BOOMING ENGINE ──────────────────────────────────────
# Reuses already-fetched zacks_sectors + cot_futures to compute a
# Booming/Favoured/Neutral/Lagging band per GICS sector.
# Freeze-safe: consumes existing scanner data, no new feed.
# ───────────────────────────────────────────────────────────────────────────
def compute_sector_booming(zacks_sectors, cot_futures):
    """
    Input:  zacks_sectors dict (from data['zacks_sectors'])
            cot_futures dict   (from data['cot_futures'])
    Output: list of dicts sorted by score desc:
    { sector, score, band, zacks_signal, cot_signal,
      top_tickers, decisive_signal }
    Band: Booming ≥70 / Favoured ≥50 / Neutral ≥30 / Lagging <30
    """
    # COT mapping — only sectors with real futures (SX-5 honesty rule)
    COT_MAP = {
        'Energy':           'Crude',
        ...
        'Financials':       'SP500',
        'Industrials':      'SP500',
        # All others: no direct futures proxy → zero weight
    }

    results = []
    for sector, zdata in zacks_sectors.items():
        # ── Signal 1: Zacks concentration (40 pts max) ──────────────────
        # pct_top = % of sector stocks at #1/#2 rank
        pct_top   = zdata.get('pct_top', 0)
        rank1     = zdata.get('rank1', 0)
        # Score: more #1s = stronger
        zacks_pts = min(40, round(pct_top * 1.2))   # 33% → 40; 20% → 24
        zacks_sig = (
            'Strong Buy' if pct_top >= 25 else
            'Buy'        if pct_top >= 15 else
            'Neutral'    if pct_top >= 8  else
            'Weak'
        )

        # ── Signal 2: COT futures (20 pts max, only mapped sectors) ─────
        cot_key = COT_MAP.get(sector)
        cot_data = cot_futures.get(cot_key, {}) if cot_key else {}
        cot_sig  = cot_data.get('signal', 'N/A')
        cot_pts  = (
            20 if cot_sig == 'VERY BULLISH' else
            15 if cot_sig == 'BULLISH'      else
            0  if cot_sig in ('N/A', '')    else
            -5
        )

        # ── Signal 3: Rank1 count bonus (20 pts max) ─────────────────────
        rank1_pts = min(20, rank1 * 4)   # 5 rank1s → 20pts

        # ── Signal 4: Breadth tiebreak (20 pts max) ──────────────────────
        total     = zdata.get('total', 1)
        top       = zdata.get('top', 0)
        breadth   = round((top / total) * 20) if total else 0

        score = min(100, zacks_pts + cot_pts + rank1_pts + breadth)
        band  = (
            'Booming'  if score >= 70 else
            'Favoured' if score >= 50 else
            'Neutral'  if score >= 30 else
            'Lagging'
        )

        # Decisive signal
        if score >= 70:
            decisive = 'Zacks + breadth confirms' if not cot_key else 'Zacks + COT confirms'
        elif cot_pts >= 15:
            decisive = 'COT bullish'
        elif pct_top >= 25:
            decisive = 'Zacks #1 concentration'
        else:
            decisive = 'Mixed'
        ...
        results.append({
            ...
            'score':           score,
            'band':            band,
            'zacks_pct_top':   round(pct_top, 1),
            'zacks_signal':    zacks_sig,
            'cot_key':         cot_key or 'None',
            'cot_signal':      cot_sig,
            'top_tickers':     zdata.get('top_tickers', [])[:8],
            'decisive_signal': decisive,
        })

    results.sort(key=lambda x: x['score'], reverse=True)
    booming = [r for r in results if r['band'] == 'Booming']
    log(f'Sector booming: {len(booming)} Booming, '
        f'{len([r for r in results if r["band"]=="Favoured"])} Favoured '
        f'of {len(results)} sectors')
    return results
...
def main():
    ...
    # ── Phase 0-A: World LEI (OECD) ───────────────────────────────────────
    try:
        data['world_lei'] = fetch_world_lei()
    except Exception as e:
        log(f'World LEI crashed: {e}')
        data['meta']['errors'].append(f'world_lei: {e}')
        data['world_lei'] = EXISTING.get('world_lei', {})

    # ── Phase 0-B: US Diffusion Composite ─────────────────────────────────
    try:
        data['us_diffusion'] = compute_us_diffusion(data['macros']['us'])
    except Exception as e:
        log(f'US diffusion crashed: {e}')
        data['meta']['errors'].append(f'us_diffusion: {e}')
        data['us_diffusion'] = EXISTING.get('us_diffusion', {})

    # ── Phase 0-C: Country Relative Strength + Currency Split ─────────────
    try:
        data['country_rs'] = fetch_country_rs()
    except Exception as e:
        log(f'Country RS crashed: {e}')
        data['meta']['errors'].append(f'country_rs: {e}')
        data['country_rs'] = EXISTING.get('country_rs', [])

    # ── Phase 0-D: Sector Booming (reuses zacks_sectors + cot_futures) ────
    try:
        data['sector_booming'] = compute_sector_booming(
            data['zacks_sectors'], data['cot_futures'])
    except Exception as e:
        log(f'Sector booming crashed: {e}')
        data['meta']['errors'].append(f'sector_booming: {e}')
        data['sector_booming'] = EXISTING.get('sector_booming', [])
