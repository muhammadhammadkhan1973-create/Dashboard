...
LIVE_PORTFOLIO_DEFAULT = {
    'holdings': [
        {'ticker': 'SMH',  'isin': 'IE00BMC38736', 'name': 'VanEck Semiconductor UCITS ETF',                 'shares': 295.3954, 'theme': 'Semiconductors / AI',      'world_theme': 'Asia-Tech'},
        {'ticker': 'ITWN', 'isin': 'IE00B0M63623', 'name': 'iShares MSCI Taiwan UCITS ETF',                  'shares': 179,      'theme': 'Asia-Tech (Taiwan)',       'world_theme': 'Asia-Tech'},
        {'ticker': 'KSTR', 'isin': 'IE00BKPJY434', 'name': 'KraneShares ICBCCS SSE STAR Market 50 UCITS ETF','shares': 872,      'theme': 'China Tech (STAR 50)',      'world_theme': 'Emerging'},
        {'ticker': 'AINF', 'isin': 'IE000X59ZHE2', 'name': 'iShares AI Infrastructure UCITS ETF',            'shares': 2910,     'theme': 'AI Infrastructure',        'world_theme': 'Asia-Tech'},
        {'ticker': 'STOR', 'isin': 'IE000DR59CI3', 'name': 'iShares Energy Storage & Hydrogen UCITS ETF',    'shares': 1620,     'theme': 'Energy Storage / Hydrogen','world_theme': 'Thematic-Clean'},
        {'ticker': 'FLXK', 'isin': 'IE00BHZRR030', 'name': 'Franklin FTSE Korea UCITS ETF',                  'shares': 345.595,  'theme': 'Asia-Tech (Korea)',        'world_theme': 'Asia-Tech'},
        {'ticker': 'PHPM', 'isin': 'JE00B1VS3W29', 'name': 'WisdomTree Physical Precious Metals',            'shares': 125,      'theme': 'Precious Metals (hedge)',  'world_theme': 'Commodity-Bloc'},
    ],
...
def build_live_investment(data, existing):
    try:
        ...
        # ---- rolling history -> WoW (portfolio + per holding) ----
        ...
        # ---- per-ETF news: count + top headlines (best-effort, capped, wrapped) ----
        news = []
        for r in rows:
            item = {'ticker': r['ticker'], 'name': r['name'], 'count': None, 'headlines': []}
            try:
                nc = _tce_news(r['ticker'], 'us')
                item['count'] = (nc or {}).get('count')
                srcs = _tce_news_sources(r['ticker'], 'us')
                if srcs:
                    ents = _fetch_rss_entries(srcs[0][1]) or []
                    for e in ents[:3]:
                        t = (getattr(e, 'get', lambda *_: None)('title') if hasattr(e, 'get') else None) or \
                            (e.get('title') if isinstance(e, dict) else None)
                        lnk = (e.get('link') if isinstance(e, dict) else None)
                        if t:
                            item['headlines'].append({'title': t[:160], 'link': lnk})
            except Exception:
                pass
            news.append(item)
        ...
        def _lead_lag(lst, key, unit='%'):
            if not lst:
                return None, None
            return lst[0], lst[-1]

        dl_lead, dl_lag = _lead_lag(by_daily, 'daily_pct')
        wk_lead, wk_lag = _lead_lag(by_wow, 'wow')
        yt_lead, yt_lag = _lead_lag(by_ytd, 'ytd')

        def _dir(v):
            return 'growing' if (v is not None and v >= 0) else ('depleting' if v is not None else 'flat')

        daily_txt = None
        if w_daily is not None:
            daily_txt = ("Portfolio is %s today (%+.2f%%)." % (_dir(w_daily), w_daily))
            if dl_lead and dl_lag and dl_lead['ticker'] != dl_lag['ticker']:
                daily_txt += (" %s led (%+.2f%%, %s); %s lagged (%+.2f%%, %s)."
                              % (dl_lead['ticker'], dl_lead['daily_pct'], dl_lead['theme'],
                                 dl_lag['ticker'], dl_lag['daily_pct'], dl_lag['theme']))
        weekly_txt = None
        if wow_nav is not None:
            weekly_txt = ("Over the past week the book is %s (%+.2f%%)." % (_dir(wow_nav), wow_nav))
            if wk_lead and wk_lag and wk_lead['ticker'] != wk_lag['ticker']:
                weekly_txt += (" %s drove the gains (%+.2f%%); %s was the drag (%+.2f%%)."
                               % (wk_lead['ticker'], wk_lead['wow'], wk_lag['ticker'], wk_lag['wow']))
        else:
            weekly_txt = "Weekly change needs ~7 days of history to accrue (tracking started today)."
        monthly_txt = None
        if mom_nav is not None:
            monthly_txt = "Over the past month the book is %s (%+.2f%%)." % (_dir(mom_nav), mom_nav)
        else:
            monthly_txt = "Monthly change needs ~30 days of history to accrue."
        ytd_txt = None
        if w_ytd is not None and yt_lead and yt_lag:
            ytd_txt = ("Year-to-date (fund-level, value-weighted) the book is up ~%.1f%%. "
                       "Best: %s %+.1f%% (%s). Worst: %s %+.1f%% (%s)."
                       % (w_ytd, yt_lead['ticker'], yt_lead['ytd'], yt_lead['theme'],
                          yt_lag['ticker'], yt_lag['ytd'], yt_lag['theme']))
        ...
        data['live_investment'] = {
            ...
            'totals': {'ytd': w_ytd, 'ret_1y': w_1y, 'daily': w_daily, 'wow': wow_nav, 'mom': mom_nav},
            'news': news,
            'narrative': {'daily': daily_txt, 'weekly': weekly_txt, 'monthly': monthly_txt, 'ytd': ytd_txt},
        ...
        log(f"  [Live Investment] NAV ${nav:,.0f} (holdings ${holdings_usd:,.0f} + cash ${cash_usd:,.0f}); "
            f"{len(rows)} holdings priced {n_reuse} reuse/{n_resolve} resolved/{n_pending} pending; "
            f"YTD~{('%.1f%%' % w_ytd) if w_ytd is not None else 'n/a'} WoW {('%.2f%%' % wow_nav) if wow_nav is not None else 'day1'}; "
            f"tilt top={next(iter(tilt), 'n/a')} {('%.0f%%' % next(iter(tilt.values()), 0)) if tilt else ''}")
    except Exception as e:
