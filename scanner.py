...
_LT_HOLDINGS_SRC = {
    'SMH':  ('sa', 'SMH'),   'KSTR': ('sa', 'KSTR'), 'FLXK': ('sa', 'FLKR'),
    'ITWN': ('sa', 'EWT'),   'AINF': ('ishares', 'IE000X59ZHE2'),
    ...
    try:
        ...
        for h in holdings:
            ...
            try:
                ...
                rows = []
            rows = [r for r in (rows or []) if r.get('ticker')][:10]
            if rows:
                probe['holdings_ok'] += 1
            else:
                probe['holdings_miss'] += 1
            for r in rows:
                mkt, tvsym = _lt_classify(r.get('ticker'))
            ...
            for u in f.get('holdings', [])[:10]:
                log('    · [probe] %-5s %-22s w=%-5s %-8s %-14s rating=%-11s tgt=%-7s eps_nq=%s'
                    % (f['ticker'], (u['name'] or '')[:22], ('%.1f%%' % u['weight']) if u.get('weight') is not None else '—',
                       u['market'] or 'unresolved', (u['tvsym'] or u['ticker'])[:14],
