...
def probe_cot_legacy():
    ...
    for label, dsid in _COT_LEGACY_DATASETS:
        ...
        for key, (inc, exc) in _COT_LEGACY_PROBE_KW.items():
            seen = []   # (name, long, short, net, oi, date), newest-first, up to 8 distinct names
            for rec in rows:
