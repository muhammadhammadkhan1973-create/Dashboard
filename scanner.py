...
SCS_MTS_INGEST = True
MTS_DUMP_RAW = False  # the layout is locked (v1.72.0); re-arm True only to re-dump the raw table text
...
def _parse_mts(text):
    """PURE. Parse the SCS MTS Daily Report. Each data row is '<S.No> <SYMBOL> <amount Rs> <prev Rs> <change%>
    [rate rate share share category]'. Builds by_ticker (per-stock outstanding MTS financing, Rs mn), market totals
    (SUM of the amount column = the market-wide leverage gauge), top_financed, as_of. Watermark noise (single-char
    lines + >=8-char runs) stripped. Returns a dict (never raises)."""
    as_of = '?'
    ...
    market = {}
    if by_ticker:
        tot = sum(v['mts_amount_mn'] for v in by_ticker.values())
        both = [(v['mts_amount_mn'], v['mts_amount_prev_mn']) for v in by_ticker.values() if v.get('mts_amount_prev_mn')]
        ct = sum(a for a, _ in both)
        cp = sum(pv for _, pv in both)
        ...
        market = {'total_mn': round(tot, 1),
                  'total_prev_mn': round(cp, 1) if cp else None,
                  'net_change_mn': round(ct - cp, 1) if cp else None,
                  'change_pct': round((ct - cp) / cp * 100, 2) if cp else None,
                  'wavg_rate': wavg, 'n_symbols': len(by_ticker)}
    top_financed = sorted(
