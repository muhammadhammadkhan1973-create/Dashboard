...
def compute_tce_streams(ticker, market='us', spy_6mo_ret=None, prev_rev_est=None,
    ...
    try:
        ...
        # v1.157.0: retry the RSS fetch up to 3x when it comes back empty/bozo. Under the v1.156.0
        # concurrent TCE pool a transient empty response occasionally dropped a real s1_news (seen on
        # PSX FFC), which the serial run always got -- the retry restores serial parity. A genuinely
        # news-less name simply stays 0 after 3 quick attempts (staggered so a burst isn't re-throttled).
        feed = None
        for _n in range(3):
            feed = feedparser.parse(url)
        ...
        cutoff = dt.datetime.utcnow() - dt.timedelta(days=14)
        recent = 0
        for entry in ((feed.entries[:30]) if (feed is not None and getattr(feed, 'entries', None)) else []):
            try:
