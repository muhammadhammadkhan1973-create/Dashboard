...
_US_TV_COLS = ['name', 'market_cap_basic', 'exchange', 'sector',
               ...
               'close', 'price_earnings_ttm', 'debt_to_equity']
...
def fetch_bank_snapshot(symbols):
    """SCS Trade per-bank current-year snapshot (DISPLAY/CONTEXT + cross-check; NOT an IG2 series writer).
    Stores data['bank_snapshot'][ticker] = {roe, roa, eps_annual, book_value_ps, adr, equity_assets, pe, pb,
    cash_deposits, cfps, as_of}. Throttled 24h; last-good carried; fully guarded; never breaks the run."""
    ...
    if last:
        try:
            ...
            pass
    try:
        manual = json.load(open('bank_ig2_inputs.json'))
    except Exception:
        manual = {}
    out = {'_fetched_utc': dt.datetime.utcnow().isoformat()}
    diag_done = False
    for sym in symbols:
        try:
            ...
            html = r.text
            if not diag_done:
                k = html.lower().find('return on equity')
                if k >= 0:
                    snip = re.sub(r'\s+', ' ', re.sub(r'<[^>]+>', ' ', html[k:k + 220]))[:150]
                    log(f'    [Wave Q snapshot diag] {sym} raw around "Return On Equity": {snip}')
                diag_done = True
            rec = {}
            for key, label in _BANK_SNAP_FIELDS:
                v = _scs_field(html, label)
                if v is not None:
                    rec[key] = v
            if rec:
                rec['as_of'] = '2025'  # SCS "Last Annual ... Upto 2025 4Q"; refined when the PDF series parser lands
                out[sym] = rec
                # cross-check vs manual IG2 ADR (data-quality QA; logging-only)
                g = (manual or {}).get(sym) or {}
                gl = (g.get('gross_loans') or {}).get('2024'); dp = (g.get('deposits') or {}).get('2024')
                if gl and dp and rec.get('adr') is not None:
                    man_adr = gl / dp * 100.0
                    if abs(man_adr - rec['adr']) > 8:
                        log(f'    [Wave Q snapshot xcheck] {sym}: manual ADR {man_adr:.1f}% vs SCS {rec["adr"]:.1f}% (delta>8 — review)')
            time.sleep(1.2)
        except Exception as e:
            log(f'    [Wave Q snapshot] {sym} skipped ({type(e).__name__})')
    n = len(out) - 1
    if n <= 0:
        log('  [Wave Q snapshot] 0 banks parsed — carrying last-good')
        return prev or out
    sample = next((k for k in out if k != '_fetched_utc'), None)
    log(f'  [Wave Q snapshot] SCS current-year: {n}/{len(symbols)} banks parsed (e.g. {sample}: {out.get(sample)})')
    return out
...
def fetch_bank_sector_kpmg():
    ...
    try:
        ...
        if len(got) < 2:
            log('  [Wave Q sector] <2 fields parsed — carrying last-good')
            return prev or out
        return out
    except Exception as e:
...
def classify_explosive(cond, partial_ok=False):
    ...
    accel  = bool(cond['a1'])
    cash_fail = (cond['c1'] is False and cond['c3'] is False)
    if growth and accel and not cash_fail:  return 'EXPLOSIVE — both signals'
    if growth and accel and cash_fail:       return 'QUALITY-GROWTH (growth + accel, cash flow does not back earnings)'
...
def run_explosive(candidates, market='us'):
    ...
    out.sort(key=lambda r: (str(r.get('verdict', '')).startswith('EXPLOSIVE'),
