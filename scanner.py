...
_SECTOR_MED_COLS = ['sector', 'price_earnings_ttm', 'operating_margin', 'net_margin', 'return_on_equity']
...
def fetch_sector_medians(sample=2000):
    ...
    try:
        ...
        for d in rows:
            ...
            sec, pe, opm, npm, roe = vals
            if not sec:
                continue
            a = acc.setdefault(sec, {'pe': [], 'op_margin': [], 'net_margin': [], 'roe': []})
        ...
        for sec, a in acc.items():
            ...
            if any(k in rec for k in ('pe', 'op_margin', 'net_margin', 'roe')):
        ...
        if out:
            ...
            log(f'  [sector medians] {len(out)} sectors -> sector_medians.json (e.g. ' +
                ', '.join(f"{s} PE~{v.get('pe')}/ROE~{v.get('roe')}" for s, v in _ex) + ')')
            return out
...
PSX_TOP_N     = 25
PSX_HOLDINGS  = ['OGDC', 'PPL', 'MCB', 'FFC', 'HUBC']   # D2: tracked book — force-included even above the band
...
# {"psx_watchlist":[...]}. Force-included exactly like PSX_HOLDINGS: any listed name present in the
...
def derive_psx_candidates(rows, mcap_min=PSX_MCAP_MIN, mcap_max=PSX_MCAP_MAX, top_n=PSX_TOP_N):
    ...
    D2: the tracked holdings (PSX_HOLDINGS) are FORCE-INCLUDED even when they sit ABOVE the band ceiling
    (they're KSE-30 mega-caps the band excludes), so the book always gets the analyst overlay + TCE
    scoring. Pure + unit-tested (the force-include reads only PSX_HOLDINGS + the scan rows)."""
    ...
    for h in list(dict.fromkeys(PSX_HOLDINGS + PSX_WATCHLIST)):   # F5: holdings + broker watchlist, deduped
        if h in have:
            continue
        r = by_ticker.get(h)
