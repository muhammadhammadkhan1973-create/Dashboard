...
COT_LEGACY_PROBE = True  # PRE-ARMED True for ONE run: dump the legacy-combined Non-Commercial dataset/columns/contract-names + CHECK vs validated Sarmaaya ground truth, then lock the repoint in v1.107.0 and flip False
_COT_LEGACY_DATASETS = [
    ('legacy-futures-only', '6dca-aqww'),   # CFTC Commitments of Traders - legacy combined, futures-only
    ('legacy-fut+options',  'jun7-fc8e'),   # CFTC legacy combined, futures-and-options
]
# contract -> candidate substrings expected in the legacy market_and_exchange_names (the exact variant
# we validated: E-MINI S&P, NASDAQ Cons, E-MINI RUSSELL, DJIA Cons). The dump prints the ACTUAL distinct
# names that match so the right variant keyword is LOCKED from the runner, never asserted here.
_COT_LEGACY_PROBE_KW = {
    'SP500':       ['S&P 500', 'E-MINI S&P'],
    'NASDAQ':      ['NASDAQ'],
    'Russell':     ['RUSSELL'],
    'DJIA':        ['DJIA', 'DOW JONES'],
    '10yr':        ['10 YEAR', '10-YEAR', 'UST 10Y', '10Y NOTE'],
    'VIX':         ['VIX'],
    'Crude':       ['CRUDE OIL', 'WTI'],
    'NatGas':      ['NAT GAS', 'NATURAL GAS', 'HENRY'],
    'Agriculture': ['CORN'],
}
...
def probe_cot_legacy():
    ...
    for label, dsid in _COT_LEGACY_DATASETS:
        ...
        for key, kws in _COT_LEGACY_PROBE_KW.items():
            seen = []   # (name, long, short, net, oi, date), newest-first, up to 4 distinct names
            for rec in rows:
                nm = str(rec.get('market_and_exchange_names', '')).upper()
                if any(kw in nm for kw in kws) and nm not in [s[0] for s in seen]:
                    try:
                    ...
                    if len(seen) >= 4:
                        break
            if not seen:
                log(f'      {key:12}: NO legacy contract name matched {kws}')
                continue
            log(f'      {key:12}: {len(seen)} distinct name(s) matched {kws}:')
