...
_EXPLOSIVE_STMT_CACHE_FILE = str(Path(__file__).parent / 'explosive_stmt_cache.json')
def _load_explosive_cache():
    try:
        with open(_EXPLOSIVE_STMT_CACHE_FILE) as _f:
            _d = json.load(_f)
...
def _save_explosive_cache(cache):
    try:
        with open(_EXPLOSIVE_STMT_CACHE_FILE, 'w') as _f:
            json.dump(cache, _f)
    except Exception as _e:
        log(f'  [Explosive cache] save failed: {_e}')
...
def run_explosive(candidates, market='us'):
    ...
    if market == 'us':
        if _cache_dirty:
            _save_explosive_cache(_stmt_cache)
        log(f'  [Explosive cache] {_cache_hits} hit / {_cache_fetched} fetched '
            f'(income_stmt cached {EXPLOSIVE_STMT_CACHE_DAYS}d)')
