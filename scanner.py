...
def build_us_multibagger(survivors):
    """Phase M-1 live US multibagger list. Screens the small-cap growth survivors (already
    $300M-$2bn + revenue-growth-gated by the US screen), caps the SEC CFO/CPAT pull for runtime, scores
    each in Python, and RANKS gate-passers by CFO/CPAT DESCENDING ('cash is king'). Freeze-safe."""
    MB_MAX_SEC = 30
    MB_DISPLAY = 15
    pool = [s for s in (survivors or [])
            if s.get('market_cap_m') is not None and s['market_cap_m'] <= 10000]
    pool.sort(key=lambda s: (s.get('rev_growth') if s.get('rev_growth') is not None else -1e9),
              reverse=True)
    scored, sec_calls = [], 0
    for s in pool[:max(MB_MAX_SEC, MB_DISPLAY)]:
        cc = None
        if sec_calls < MB_MAX_SEC:
            try:
                cc = _sec_cfo_cpat(s.get('ticker'))
            except Exception:
                cc = None
            sec_calls += 1
        scored.append(score_multibagger(s, cc, 'us'))

    def rank_key(x):
        r = x.get('cfo_cpat')
        return (0 if x.get('eligible') else 1,        # gate-passers first
                0 if r is not None else 1,            # cash-confirmed first
                -(r if r is not None else 0),         # MAX CFO/CPAT at the top
                -(x.get('total') or 0),
                (x.get('market_cap_m') or 1e12))
    scored.sort(key=rank_key)
    top = scored[:MB_DISPLAY]
    _top_r = top[0].get('cfo_cpat') if top else None
    log(f'  [Multibagger US] scored {len(scored)} survivors ({sec_calls} SEC CFO/CPAT pulls); '
        f'{sum(1 for x in scored if x.get("eligible"))} gate-passing; top CFO/CPAT={_top_r}')
    return top
