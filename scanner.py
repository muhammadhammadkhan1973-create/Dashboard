...
_ETF_CATALOG['Equity Pakistan'] = [
    ...
    {'name': 'Chimera S&P Pakistan UCITS ETF (Lunate, ADX)', 'ter': 1.00, 'ytd': 28.12, 'size_m_eur': 3,
     'isin': 'IE00088ID6F8', 'dist': 'Distributing', 'holdings': 'OGDC, UBL, MCB, Lucky Cement, HBL, FFC'},
...
def fetch_cot_futures():
    ...
    # v1.217.0: the runtime [vs Sarmaaya] drift check is RETIRED. It compared each run's LIVE net
    # position against a snapshot frozen on 2026-06-16 -- positions move every week, so 'DRIFT' was
    # guaranteed and meaningless; and the owner no longer has Sarmaaya access, so the reference can
    # never be refreshed. The BASIS lock is unaffected: v1.106-1.108 PROVED this dataset/bucket
    # (legacy combined Non-Commercial, 6dca-aqww, futures-only) reproduces Sarmaaya exactly -- that
    # proof doesn't expire. CFTC is the official primary source; Sarmaaya itself derives from it.
    rows = []
    try:
...
def fetch_foundation_universe():
    ...
    try:
        ...
        _cols = _US_TV_COLS + ['description', 'Perf.YTD', 'Perf.Y', 'Perf.5Y',
                               # v1.209.0: forward-estimate + analyst-mark + volume-average columns
                               # (same single scan) -> the estimate-revision clock + T4 volume family.
        ...
        while start < FOUNDATION_UNIVERSE_CAP:
            ...
            for d in batch:
                ...
                recs.append({
                    ...
                    'price':         price,
                    # v1.209.1: raw estimate/volume fields must ride the NAMED record (the v1.209.0 miss)
                    'earnings_per_share_forecast_next_fq': rec.get('earnings_per_share_forecast_next_fq'),
                    'recommendation_mark':                 rec.get('recommendation_mark'),
                    'average_volume_10d_calc':             rec.get('average_volume_10d_calc'),
                    'average_volume_60d_calc':             rec.get('average_volume_60d_calc'),
...
_US_ANALYST_COLS = ['name', 'close', 'price_target_average', 'recommendation_mark',
                    ...
                    'earnings_per_share_fq', 'sector',
                    'Perf.YTD', 'Perf.Y', 'SMA50', 'SMA200',
                     'description']  # v1.219.1: appended LAST (byte-align precedent) -- real company names for look-through rows   # v1.89.0: 'sector' APPENDED (last) so the zip stays aligned and derive_psx_analyst_streams (reads by key) is byte-identical -> s11/s12/s9 unchanged (freeze-safe); it lets the ETF-consensus large-caps inherit the TV sector the scan already returns. v1.214.1: Perf.YTD/Perf.Y/SMA50/SMA200 further appended (same reason -- keys after 'sector' are unread by the PSX/TCE consumers) to give the look-through US underlyings a live trend.
...
# ==================== Wave PSX-DPS: PSX Data Portal full-universe live feed ====================
# CONFIRMED 2026-07-12 (my own web_fetch, not a runner probe): dps.psx.com.pk/market-watch returns a STATIC,
# parseable HTML table of the FULL live PSX universe (~470 rows) with columns:
#   SYMBOL | SECTOR(code) | LISTED IN(index memberships) | LDCP | OPEN | HIGH | LOW | CURRENT | CHANGE | CHANGE% | VOLUME
# This is the FIRST direct-PSX source proven reachable from a datacenter fetch (memory's dps.psx-blocked flag
# was from the RETIRED Apps Script bot, never the current path). It fixes F5 (hardcoded watchlist -> real full
# universe) and gives a TradingView-INDEPENDENT PSX price + index-membership feed. ADDITIVE: does NOT replace the
# TV Pakistan scan that drives screening -- it is a parallel confirmed feed. Fully guarded: any miss -> {} and
# the run proceeds unchanged on the TV path (so if GitHub's runner IP is blocked where my fetch was not, nothing
# breaks). DISPLAY/DATA feed; no scoring effect.
PSX_DPS_INGEST = True
_PSX_DPS_URL = 'https://dps.psx.com.pk/market-watch'
# row cell order confirmed from the live page (status flags NC/XD/WU/XB may trail the symbol link text)
_PSX_DPS_ROW_RE = re.compile(
    r'/company/([A-Z0-9]+)"[^>]*>.*?</a>\s*(?:[A-Z]{2})?\s*</td>'      # symbol (skip trailing NC/XD/etc.)
    r'\s*<td[^>]*>\s*(\d{3,4})\s*</td>'                               # sector code
    r'\s*<td[^>]*>([^<]*)</td>'                                          # listed-in (index memberships, may be empty for ETFs)
    r'(?:\s*<td[^>]*>\s*([\d,]+\.?\d*)\s*</td>){5}'                  # LDCP OPEN HIGH LOW CURRENT (take CURRENT = 5th)
    , re.I | re.S)


def _psx_dps_num(s):
    """PURE. '1,038.82' -> 1038.82; '' / '-' -> None. Never raises."""
    if not s:
        return None
    try:
...
def _parse_psx_dps(html):
    """PURE. Parse the dps.psx market-watch table into {SYMBOL: {price, ldcp, open, high, low, change, change_pct,
    volume, sector, indices:[...]}}. Row-by-row on the confirmed 11-column structure; a malformed row is skipped,
    never fatal. Returns {} on total miss. Built against the REAL 2026-07-12 markup, not a guess."""
    if not html:
        return {}
    out = {}
    # split on the per-company anchor so each chunk is one row; robust to whitespace/attr drift
    parts = re.split(r'(?=/(?:company|etf)/[A-Z0-9]+")', html)
    for chunk in parts:
        m = re.match(r'/(?:company|etf)/([A-Z0-9]+)"', chunk)
        if not m:
            continue
        sym = m.group(1)
        # pull the numeric <td> cells in order after the LISTED-IN column
        sector_m = re.search(r'<td[^>]*>\s*(\d{3,4})\s*</td>', chunk)
        # index memberships: the cell right after the sector code, comma-joined tokens like KSE100,KMI30
        idx_m = re.search(r'<td[^>]*>\s*(\d{3,4})\s*</td>\s*<td[^>]*>([A-Z0-9,]*)</td>', chunk)
        nums = re.findall(r'<td[^>]*>\s*(-?[\d,]+\.?\d*%?)\s*</td>', chunk)
        # after sector(1) + listed-in, the numeric block is LDCP OPEN HIGH LOW CURRENT CHANGE CHANGE% VOLUME
        # nums includes the sector code as first numeric; drop it, then map the 8 trading cells
        vals = [n for n in nums]
        # find the CURRENT (5th price after ldcp/open/high/low) by locating the price run
        price_run = re.findall(r'<td[^>]*>\s*([\d,]+\.\d{2})\s*</td>', chunk)  # ldcp/open/high/low/current all have 2dp
        cur = _psx_dps_num(price_run[4]) if len(price_run) >= 5 else None
        if cur is None:
            continue
        chg_m = re.search(r'<td[^>]*>\s*(-?[\d,]+\.\d+)\s*</td>\s*<td[^>]*>\s*(-?[\d.]+)%\s*</td>', chunk)
        vol_m = re.search(r'([\d,]{4,})\s*</td>\s*(?:</tr>|$)', chunk)
        idx_raw = (idx_m.group(2) if idx_m else '') or ''
        out[sym] = {
            'price': cur,
            'ldcp': _psx_dps_num(price_run[0]) if len(price_run) > 0 else None,
            'open': _psx_dps_num(price_run[1]) if len(price_run) > 1 else None,
            'high': _psx_dps_num(price_run[2]) if len(price_run) > 2 else None,
            'low':  _psx_dps_num(price_run[3]) if len(price_run) > 3 else None,
            'change':     _psx_dps_num(chg_m.group(1)) if chg_m else None,
            'change_pct': _psx_dps_num(chg_m.group(2)) if chg_m else None,
            'volume':     int(vol_m.group(1).replace(',', '')) if vol_m else None,
            'sector':  sector_m.group(1) if sector_m else None,
            'indices': [t for t in idx_raw.split(',') if t],
        }
    return out


def fetch_psx_market_watch():
    """Wave PSX-DPS. Fetch dps.psx.com.pk/market-watch -> full live PSX universe.
    CONFIRMED reachable+static from a datacenter fetch. Guarded; any miss -> {} (TV path unaffected)."""
    if not PSX_DPS_INGEST:
        return {}
    try:
        r = requests.get(_PSX_DPS_URL, headers={'User-Agent': UA, 'Accept': 'text/html,*/*'}, timeout=25)
        if r.status_code != 200:
            log(f'  [Wave PSX-DPS] skip (HTTP {r.status_code}) -> TV path unaffected')
            return {}
        parsed = _parse_psx_dps(r.text or '')
    except Exception as e:
        log(f'  [Wave PSX-DPS] FETCH FAILED ({type(e).__name__}: {str(e)[:60]}) -> TV path unaffected')
        return {}
    if not parsed:
        log('  [Wave PSX-DPS] 0 rows parsed (structure drift or IP-block on runner) -> TV path unaffected')
        return {}
    # sanity: a few of the owner's core names should be present with plausible prices
    _core = {k: parsed[k]['price'] for k in ('OGDC', 'HUBC', 'FFC', 'MCB', 'UBL') if k in parsed}
    _kse100 = sum(1 for v in parsed.values() if 'KSE100' in v.get('indices', []))
    log(f'  [Wave PSX-DPS] {len(parsed)} symbols from dps.psx market-watch (KSE100 members {_kse100}); core px {_core}')
    return parsed
# ================== end Wave PSX-DPS ==================
...
# ── ETF SCREENER PROBE (v1.214.4) ─────────────────────────────────────────────────────────────────
# ONE-SHOT, LOGGING-ONLY, FREEZE-SAFE runner probe answering: can we LIVE-SCREEN UCITS ETFs by return on
# the same scanner.tradingview.com/{market}/scan endpoint the resolver already uses? If yes, the live
# self-discovering World-ETF engine can replace the manual quarterly ETF_Universe.xlsx upload. Mirrors the
# PROVEN Foundation-Universe screener body (columns/filter/sort/range/markets POST). Sandbox blocks TV, so
# the answer comes from the runner log. Touches NO data/scoring/screening/IM3/TCE/index -> canaries unchanged.
ETF_SCREENER_PROBE = False  # v1.215.0: probe confirmed (all 5 segments rank UCITS ETFs by return; aum/isin/Perf/expense served) -> real build_live_etf_discovery() now live; probe retired


def probe_etf_screener():
    """LOGGING-ONLY. For each European UCITS-listing segment (uk/germany/netherlands/switzerland/france):
    (A) minimal-column pass (name/close/Perf.YTD, type=fund, sort Perf.YTD desc) -> confirms the segment
        returns ETFs RANKED by return + logs the top 5 (eyeball: UCITS vs leveraged/inverse);
    (B) on the first segment that returns rows, a PER-COLUMN availability probe (append each candidate to the
        safe minimal set; HTTP 200 = TV serves it) for the fields the live build needs -- aum/nav_total
        (quality floor), isin (buyable mapping), description/currency, Perf.6M/Perf.3M -- so the engine is
        built only on columns TV actually serves. Never raises; wrapped by the caller."""
    if not ETF_SCREENER_PROBE:
        return
    import requests as _rq
    log('  [ETF screener probe] can we live-rank UCITS ETFs by return? (goal: drop the quarterly ETF_Universe.xlsx upload)')
    _markets = ['uk', 'germany', 'netherlands', 'switzerland', 'france']
    _min_cols = ['name', 'description', 'close', 'Perf.YTD']
    _first_ok = None
    for _mkt in _markets:
        try:
            _pa = {"columns": _min_cols,
                   "filter": [{"left": "type", "operation": "equal", "right": "fund"}],
                   "sort": {"sortBy": "Perf.YTD", "sortOrder": "desc"},
                   "range": [0, 10], "markets": [_mkt]}
            _r = _rq.post(f'https://scanner.tradingview.com/{_mkt}/scan',
                          json=_pa, headers={'User-Agent': UA}, timeout=30)
            _rows, _tot = [], 0
            if _r.status_code == 200:
                _j = _r.json()
                _rows = _j.get('data', []) or []
                _tot = _j.get('totalCount', len(_rows))
            log(f'    · [{_mkt}] type=fund sort Perf.YTD: HTTP {_r.status_code}  total~{_tot}  shown {len(_rows)}')
            for _d in _rows[:5]:
                _rec = dict(zip(_min_cols, _d.get('d', [])))
                log(f'        {str(_d.get("s","?")):24s} YTD={_rec.get("Perf.YTD")}  close={_rec.get("close")}  {str(_rec.get("description"))[:40]}')
            if _rows and _first_ok is None:
                _first_ok = _mkt
        except Exception as _e:
            log(f'    · [{_mkt}] minimal pass FAILED: {type(_e).__name__} {str(_e)[:70]}')
    # Pass B -- per-column availability, only on the first working segment (lean: ~8 extra calls total)
    if _first_ok:
        log(f'  [ETF screener probe] per-column availability on [{_first_ok}] (which fields does TV serve for the live build?):')
        for _cand in ['aum', 'nav_total', 'isin', 'currency', 'fundamental_currency_code',
                      'Perf.6M', 'Perf.3M', 'expense_ratio', 'typespecs']:
            try:
                _cols = _min_cols + [_cand]
                _pb = {"columns": _cols,
                       "filter": [{"left": "type", "operation": "equal", "right": "fund"}],
                       "sort": {"sortBy": "Perf.YTD", "sortOrder": "desc"},
                       "range": [0, 3], "markets": [_first_ok]}
                _r = _rq.post(f'https://scanner.tradingview.com/{_first_ok}/scan',
                              json=_pb, headers={'User-Agent': UA}, timeout=30)
                if _r.status_code == 200:
                    _rows = _r.json().get('data', []) or []
                    _v = dict(zip(_cols, _rows[0].get('d', []))).get(_cand) if _rows else None
                    log(f'        {_cand:28s} SERVED (HTTP 200)  sample={str(_v)[:44]}')
                else:
                    log(f'        {_cand:28s} not served (HTTP {_r.status_code})')
            except Exception as _e:
                log(f'        {_cand:28s} probe error: {type(_e).__name__} {str(_e)[:50]}')
    else:
        log('  [ETF screener probe] no segment returned rows with type=fund -- next probe will retry without the type filter and log the types present.')
    log('  [ETF screener probe] done. If segments return ranked UCITS ETFs with aum+isin+Perf served, the live self-discovering engine + LEI/diffusion gate can be built and the quarterly upload retired.')
...
def resolve_etf_live_price(isin):
    ...
    for market, markets_val in [('uk', ['uk']), ('germany', ['germany']), ('netherlands', ['netherlands']), ('switzerland', ['switzerland']), ('uae', ['uae'])]:  # v1.215.11: uae (ADX) appended LAST for the Chimera Pakistan fund; first live run is the probe
        try:
            payload = {
                'columns': ['name', 'isin', 'close', 'currency', 'exchange', 'Perf.YTD', 'Perf.Y', 'change', 'SMA50', 'SMA200'],
                'filter':  [{'left': 'isin', 'operation': 'equal', 'right': isin}],
            ...
            for row in rows:
                ...
                _y1  = rec.get('Perf.Y')
                _chg = rec.get('change')
                _s50 = rec.get('SMA50'); _s200 = rec.get('SMA200')  # v1.214.0: 50/200-day moving averages
                cands.append((sort_key, {'sym': row.get('s', ''), 'close': round(float(close), 4),
                                         ...
                                         'sma50': round(float(_s50), 4) if isinstance(_s50, (int, float)) else None,
                                         'sma200': round(float(_s200), 4) if isinstance(_s200, (int, float)) else None}))
            if cands:
...
MTS_SOURCE_PROBE = True   # one-shot; flip False after reading the runner verdict and wiring the winner
...
# Business Recorder MTS market-wrap language: "MTS investment ... Rs <N> billion/bn" (leverage total).
# Widest net first; the dump confirms the exact phrasing so this can be tightened to the real structure.
_BREC_MTS_RE = re.compile(
    r'(?:mts|margin[\s-]*trad\w*)[^.]{0,160}?'
    r'(?:rs\.?|pkr|rupees?)?\s*([\d,]+(?:\.\d+)?)\s*(billion|bn|million|mn|mln)\b',
    re.I | re.S)


def _brec_mts_parse(text):
    """PURE. From Business Recorder market-wrap text, extract the market-wide MTS investment TOTAL in Rs mn,
    and the freshest article date. Never raises. Returns {} if no total is found (caller then uses SCS fallback)."""
    if not text:
        return {}
    m = _BREC_MTS_RE.search(text)
    total_mn = None
    if m:
        try:
            val = float(m.group(1).replace(',', ''))
            unit = (m.group(2) or '').lower()
            total_mn = round(val * 1000.0, 1) if unit in ('billion', 'bn') else round(val, 1)
        except (ValueError, AttributeError):
            total_mn = None
    # freshest date -- reuse the probe helper (most-recent parseable date in the body)
    dstr, age = _mts_probe_freshest_date(text)
    out = {}
    if total_mn is not None:
        out = {'market': {'total_mn': total_mn}, 'as_of': dstr or '?', 'source': 'business_recorder'}
        if age is not None:
            out['as_of_age_days'] = age
            out['stale'] = bool(age > MTS_STALE_DAYS)
    return out


def fetch_brec_mts():
    """Wave PSX-R Phase-3c. Fetch the Business Recorder MTS market wrap -> market-wide MTS leverage total.
    Primary live source (probe-confirmed fresh, HTTP 200, age 0d). Guarded; any miss -> {} (SCS fallback)."""
    if not BREC_MTS_INGEST:
        return {}
    try:
        r = requests.get(_BREC_MTS_SEARCH, headers={'User-Agent': UA, 'Accept': '*/*'}, timeout=20)
        if r.status_code != 200:
            log(f'  [Wave PSX-R MTS/brec] skip (HTTP {r.status_code})')
            return {}
        text = r.text or ''
    except Exception as e:
        log(f'  [Wave PSX-R MTS/brec] FETCH FAILED ({type(e).__name__}: {str(e)[:50]})')
        return {}
    if BREC_MTS_DUMP_RAW:
        # log ~400 chars around the first MTS/margin mention so the exact total markup can be locked next pass
        low = text.lower()
        i = low.find('mts')
        if i < 0:
            i = low.find('margin')
        if i >= 0:
            snippet = re.sub(r'\s+', ' ', text[max(0, i-120):i+320])
            log('  [Wave PSX-R MTS/brec DUMP] markup around first MTS mention: ' + snippet[:440])
        else:
            log('  [Wave PSX-R MTS/brec DUMP] no MTS/margin token in page body (search may need a different query)')
    parsed = _brec_mts_parse(text)
    if not parsed.get('market', {}).get('total_mn'):
        log('  [Wave PSX-R MTS/brec] no market total parsed yet (re-read DUMP to lock the regex) -> SCS fallback')
        return {}
    _mk = parsed['market']
    log(f'  [Wave PSX-R MTS/brec] LIVE total Rs {_mk["total_mn"]}mn as-of {parsed.get("as_of")} '
        f'(age {parsed.get("as_of_age_days","?")}d, stale={parsed.get("stale","?")}) -- Business Recorder')
    return parsed
# ================== end Wave PSX-R Phase-3c Business Recorder LIVE MTS ==================
...
def _score_standard(ticker, d, bond_yield=0.043):
    ...
    for m in metrics:
        if m.get('key') in _EXCL:
            ...
            m['pts'] = 0
            m['max'] = 0
    # v1.215.14 (Wave A -- age-neutral strength grade; owner decision 2026-07-11):
    # (1) trend-window honesty: the ten 3yr-vs-5yr comparison metrics can NEVER verdict GOOD when
    #     the underlying series has <=3 non-None annual values (both windows then average the SAME
    #     values, so the strict inequality is structurally false) -- 'trend not measurable' was
    #     being scored as 'trend mediocre' (WATCH, 60% of weight). Such metrics now verdict NA.
    # (2) adaptive denominator: NA strength metrics are EXCLUDED from the max (matching the
    #     canonical im3_score.py per-name adaptive maximum, e.g. IAG 106/114, and the v1.201.0
    #     EXCL precedent) -- a young filer scores on what its filings can prove and is never
    #     docked for years that do not exist yet. Audit rows keep verdict NA + nominal max for
    #     transparency; only the denominator sum skips them. Banks (System-B) and the valuation
    #     indicator are untouched.
    _TREND_SERIES = {'cfo_trend': cfo_s, 'nfa_turn': nfat_s, 'eps_trend': eps_,
                     'val_shareholders': eps_, 'inv_turn': it_s, 'dro': dro_s,
                     'fat': nfat_s, 'ccc': ccc_s, 'fcf_trend': fcf_s, 'fcf_cfo': fcf_cfo_s}
    for m in metrics:
        _ts = _TREND_SERIES.get(m.get('key'))
        if _ts is not None and m['verdict'] == 'WATCH' \
           and len([x for x in _ts if x is not None]) <= 3:
            m['verdict'] = 'NA'
            m['pts'] = 0
    total = sum(m['pts'] for m in metrics if not m.get('val'))
    max_s = sum(m['max'] for m in metrics if not m.get('val') and m['verdict'] != 'NA')
    ...
    try:
        ...
        def _compose(cands, mk, tce_rows, cash_w, extra_tilt):
            ...
            for c in cands:
                ...
                raw[tk] = max(float(s), 1.0) * m * tmult * xmult
        ...
        data['pb'] = {'as_of': today, 'us': us_book, 'psx': px_book, 'track': tr,
                      'note': 'Paper model portfolio built by fixed rules from the live engines. '
                              'Not orders. Backtest half arrives when the history store deepens.'}
...
def build_bottom_up_sectors(foundation_universe, sector_booming=None, ret_field='perf_ytd'):
    ...
    agg = defaultdict(lambda: {'n': 0, 'winners': 0, 'rets': [], 'top': []})
    for r in fu:
...
def _li_fx(data, existing):
    ...
    try:
        got = fetch_index_tv({'gbpusd': 'FX_IDC:GBPUSD', 'eurusd': 'FX_IDC:EURUSD'}, markets=('forex',))
        fx['GBP'] = (got.get('gbpusd') or {}).get('px') or prev.get('GBP') or 1.34
        fx['EUR'] = (got.get('eurusd') or {}).get('px') or prev.get('EUR') or 1.16
    except Exception:
        fx['GBP'] = prev.get('GBP') or 1.34
        fx['EUR'] = prev.get('EUR') or 1.16
    ...
    try:
        ...
        for t, r in pool.items():
            ...
            if r['lane'] == 'speculative':
                ...
                if fams >= 2:
                    turnaround.append(dict(base, n_signals=fams, signals=sig))
            else:
                sig = []
                ...
                if len(sig) >= 2:
                    quality.append(dict(base, n_signals=len(sig), signals=sig))
    ...
    try:
        ...
        for r in ex:
            ...
            base = {'ticker': t, 'name': im3.get('name') or r.get('name') or t,
                    ...
                    'keystone': r.get('strength_score'), 'perf_6m': p6, 'perf_3m': p3,
        ...
        data['psx_m2_watch'] = {
            ...
            'watch_turnaround': t_shown, 'watch': t_shown,
            'families': ['F1 price turn', 'F2 fundamentals inflecting', 'F3 screen verdict',
...
def main():
    ...
    try:
        _live_disc = build_live_etf_discovery(min_aum_m=50.0)   # v1.215.3: $50M floor (aum is absolute currency; range $7.7M..$36.8bn) drops tiny/illiquid funds, keeps every real candidate
    except Exception as _e:
    ...
    try:
        build_live_investment(data, EXISTING)  # v1.211.0: owner's live IBKR ETF portfolio
