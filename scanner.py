...
def _attach_discovery_holdings(data, existing=None):
    ...
    for d in disc:
        ...
        hs = hmap.get(iz) or _hstr(cache.get(iz))
        if hs:
            d['holdings'] = hs
            reuse += (0 if cache.get(iz) and iz not in hmap else 1)
        else:
            uncovered.append(d)
    ...
    log(f'  [discovery holdings] {reuse} reuse (xlsx/watchlist) + {fetched} fetched ({ish} iShares + {jet} justETF, cached) '
