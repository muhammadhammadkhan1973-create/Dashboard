...
def fetch_bank_snapshot(symbols):
    ...
    has_cached = any(k != '_fetched_utc' for k in prev)   # real banks present? (a failed run stamps but caches nothing)
    ...
    for sym in symbols:
        try:
            ...
            if not series:
                continue
            out[sym] = {'series': series, 'as_of': '?'}   # RAW; basis/order pending validation vs a known bank
            if dumped < 2:
                shown = ', '.join(f'{k}(n={len(v)},last={v[-1]})' for k, v in list(series.items())[:12])
                log(f'    [Wave Q snapshot map] {sym}: {shown}')
                dumped += 1
            if sym == 'AKBL':
                roe = next((v for k, v in series.items() if 'return on equity' in k.lower() and v), None)
                if roe:
                    ok = abs(roe[-1] - 15.03) < 3
                    log(f'    [Wave Q snapshot validate] AKBL ROE series latest={roe[-1]} vs known 15.03 -> '
                        + ('MATCH (basis/order OK -> safe to map in v1.56)' if ok
                           else 'MISMATCH (chart basis/order differs -> do NOT map)'))
            time.sleep(1.2)
        except Exception as e:
