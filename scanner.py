...
def build_live_investment(data, existing):
    try:
        ...
        for h in holdings_cfg:
            ...
            if not rec:
                try:
                    rec = resolve_etf_live_price(iz); src = 'resolved'
                    if rec:
                        n_resolve += 1
                except Exception:
                    rec = None
        ...
        log(f"  [Live Investment] NAV ${nav:,.0f} (cost ${cost_total or 0:,.0f}); "
            ...
            f"({_pnl_s}); "
            f"{len(rows)} holdings {n_reuse} reuse/{n_resolve} resolved/{n_pending} pending; "
            f"day {('%+.2f%%' % day_nav) if day_nav is not None else 'n/a'}; "
