...
def fetch_cot_futures():
    """Index/commodity COT contracts used to gate US sector selection:
    SP500, NASDAQ, VIX, 10yr (TFF asset-manager) + Crude, Natural Gas, Agriculture/Corn
    (disaggregated managed-money).  Natural Gas -> Utilities, Corn -> Consumer Staples,
    Crude -> Energy (index v5.82 cotFor mapping).  Returns {contract: {long, short, net,
    signal, date}}.  Never raises (per-block guarded + last-good fallback)."""
    out = {}
    headers = {'User-Agent': UA}
    ...
    # --- Disaggregated (Managed Money): Crude, Natural Gas, Agriculture (Corn) ---
    try:
        url = ('https://publicreporting.cftc.gov/resource/72hh-3qpy.json'
               '?$order=report_date_as_yyyy_mm_dd DESC&$limit=600')
        rows = _retry_get(url, headers=headers, timeout=(8, 12), tries=3, base=1.0).json()
        # key -> (ALL-of keywords on market_and_exchange_names, very-bullish |net| threshold)
        _dis = {
            'Crude':       (('WTI-PHYSICAL', 'NEW YORK'),           200000),
            'NatGas':      (('NAT GAS NYME', 'NEW YORK MERCANTILE'), 100000),
            'Agriculture': (('CORN', 'CHICAGO BOARD OF TRADE'),     200000),
        }
        _dser = {k: [] for k in _dis}
        for rec in rows:
            name = str(rec.get('market_and_exchange_names', '')).upper()
            for key, (kws, thr) in _dis.items():
                if all(kw in name for kw in kws):
                    ...
                    net = lng - sht
                    _dser[key].append(net)
                    if key not in out:   # first (latest) row defines the headline
                        out[key] = {'long': int(lng), 'short': int(sht), 'net': int(net),
                                    'signal': ('VERY BULLISH' if net > thr else 'BULLISH' if net > 0
                                               else 'BEARISH' if net > -thr else 'VERY BEARISH'),
                                    'date': rec.get('report_date_as_yyyy_mm_dd')}
        for key in _dis:                  # backfilled WoW/MoM/QoQ net trend from the DESC rows
            if key in out:
                for _k, _v in _series_trend(list(reversed(_dser[key]))[-14:]).items():
                    out[key]['net_'+_k] = _v
        _dfound = [k for k in _dis if k in out]
        if 'NatGas' not in _dfound:
            # v1.102.0 dump-then-lock: the NatGas key never matches (only Crude+Agriculture do every
            # run), so the ('NATURAL GAS','NEW YORK MERCANTILE') keyword pair isn't in the CFTC contract
            # name. Log the ACTUAL disaggregated contract names mentioning GAS/HENRY so the exact string
            # is read off the runner and the right keyword locked next increment. Logging-only.
            _gas_names = []
            for rec in rows:
                _nm = str(rec.get('market_and_exchange_names', '')).upper()
                if ('GAS' in _nm or 'HENRY' in _nm) and _nm not in _gas_names:
                    _gas_names.append(_nm)
                if len(_gas_names) >= 12:
                    break
            log(f'  [COT NatGas diag] {len(rows)} rows scanned; contracts mentioning GAS/HENRY: '
                f'{_gas_names if _gas_names else "NONE (raise $limit or NatGas absent from disaggregated)"}')
        log(f'  ✓ COT futures disaggregated (Managed Money): {_dfound if _dfound else "none found"}')
    except Exception as e:
        warn(f'COT futures (disaggregated) failed: {e}')
    # last-good fallback
    if not out:
...
def screen_us_universe():
    ...
    if eps_missing:
        ...
        if FMP_KEY:
            ...
            log('    [FMP EPS] FMP_API_KEY not set — using Yahoo income_stmt only')
...
def track_shortlists(data, existing):
    ...
    try:
        ...
        # v1.103.0 (Wave PB readiness): the conviction signal AT first-seen. A name re-scores every
        # run, so its entry-day grade/tier is unrecoverable later -> this is the one Wave-PB input that
        # cannot be backfilled, so it is FROZEN here at first listing and never overwritten. Guarded:
        # any missing field is simply absent (never fabricated; existing pre-1.103.0 rows stay as-is).
        _macros = (data.get('macros') or {})
        def _entry_signal(row, mkt):
            sig = {}
            try:
                ...
                b = ((_macros.get('us') or {}).get('sp500') if mkt == 'US'
                     else (_macros.get('psx') or {}).get('kse100'))
                if isinstance(b, (int, float)) and b > 0:  sig['entry_bench'] = round(float(b), 2)
            except Exception:
                pass
            return sig
        ...
        for tab, mkt, pmap, rows in feeds:
            for row in rows:
                ...
                if rec and rec.get('first_price'):
                    ...
                    # durable alpha: pick move minus its benchmark over the same window, from the
                    # frozen entry_bench (robust where a runtime history-join would miss the entry day)
                    ...
                    _new.update(_entry_signal(row, mkt))   # freeze conviction + benchmark at entry
        ...
        n_sig  = sum(1 for v in stocks.values() if ('entry_grade' in v or 'entry_tier' in v or 'entry_bench' in v))
        log(f'  [Wave T shortlist] {len(stocks)} stock-rows tracked ({n_live} live, {n_sig} entry-signal stamped) across '
