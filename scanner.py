...
ETF_FEED_PROBE = True   # PRE-ARMED for ONE run; read the [ETF feed probe] block, lock the TV price + stockanalysis holdings method per ISIN, then flip False
...
def probe_etf_isin_feeds():
    ...
    for isin, label in _ETF_PROBE_ISINS:
        ...
        log(f'    - {isin} {label[:30]:30} | px {price if price is not None else "-"} [{tvsym or "no-symbol"}]'
...
def screen_psx_universe():
    ...
    try:
        ...
        probe_etf_isin_feeds()              # World ETF Engine Phase 6 probe (logging-only): ISIN -> TV price + stockanalysis holdings (NO Yahoo/FMP)
    except Exception as _e:
