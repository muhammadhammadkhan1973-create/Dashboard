...
def build_explosive_etf_tracker(data):
    """v1.182.0 -- World ETF Engine: prices the distinct UCITS proxies of the Explosive picks ONCE,
    then (a) ENRICHES each pick's ucits_proxy with ytd/ret_1y/daily_chg/live_price so the Explosive
    tab (Tab 10) can show ISIN + YTD + 1Y per name, and (b) builds data['explosive_etf_tracker'] =
    the BOTH-SIGNAL ('EXPLOSIVE -- both signals') subset grouped by proxy for the Results tab (Tab 14),
    each with YTD/1Y/daily. Uses resolve_etf_live_price() (now also returns daily 'change'). Freeze-safe/
    data-only: reads verdicts+proxies already computed; must run AFTER attach_ucits_proxies()."""
    def _is_both(r):
        v = (r.get('verdict') or '').upper()
        return 'EXPLOSIVE' in v and 'NOT' not in v
    recs_all = [r for r in ((data.get('explosive_us') or []) + (data.get('explosive_psx') or [])) if isinstance(r, dict)]
    # 1) price each distinct proxy ISIN once (across ALL positive-verdict picks that carry a proxy)
    price_cache = {}
    for _r in recs_all:
    ...
    # 2) enrich every pick's ucits_proxy with the priced perf -> Explosive tab shows ISIN + YTD + 1Y
    _n_enriched = 0
    for _r in recs_all:
        ...
        if _lp:
            _p['ytd']        = _lp.get('ytd')
            _p['ret_1y']     = _lp.get('ret_1y')
            _p['daily_chg']  = _lp.get('change')
            _p['live_price'] = _lp.get('close')
            _p['live_sym']   = _lp.get('sym')
            _p['live_ccy']   = _lp.get('currency')
            _n_enriched += 1
    # 3) both-signal tracker for the Results tab (grouped by proxy)
    groups = {}
    for _mkt, _key in (('us', 'explosive_us'), ('psx', 'explosive_psx')):
        for _r in (data.get(_key) or []):
            if not (isinstance(_r, dict) and _is_both(_r)):
                continue
            _p = _r.get('ucits_proxy') or {}
            _isin = _p.get('isin')
            if not _isin:
                continue
            g = groups.setdefault(_isin, {'name': _p.get('name'), 'ter': _p.get('ter'),
                                          'tier_label': _p.get('tier_label'), 'us': [], 'psx': []})
            g[_mkt].append(_r.get('ticker'))
    out = []
    for _isin, g in groups.items():
        _lp = price_cache.get(_isin) or {}
        out.append({'isin': _isin, 'name': g['name'], 'ter': g['ter'], 'tier_label': g['tier_label'],
                    'stocks_us': g['us'], 'stocks_psx': g['psx'], 'n_stocks': len(g['us']) + len(g['psx']),
                    'live_price': _lp.get('close') if _lp else None, 'live_sym': _lp.get('sym') if _lp else None,
                    'live_ccy': _lp.get('currency') if _lp else None, 'live_exch': _lp.get('exchange') if _lp else None,
                    'ytd': _lp.get('ytd') if _lp else None, 'ret_1y': _lp.get('ret_1y') if _lp else None,
                    'daily_chg': _lp.get('change') if _lp else None})
    out.sort(key=lambda r: (-r['n_stocks'], -(r['ytd'] if isinstance(r['ytd'], (int, float)) else -999)))
    data['explosive_etf_tracker'] = out
    _priced = sum(1 for v in price_cache.values() if v)
    log(f'  [Explosive->ETF tracker] priced {_priced}/{len(price_cache)} distinct UCITS proxy ETF(s); '
        f'enriched ucits_proxy (ISIN+YTD+1Y) on {_n_enriched} pick(s); {len(out)} proxy group(s) '
        f'tracking {sum(r["n_stocks"] for r in out)} both-signal name(s) on the Results tab')
...
def main():
    ...
    build_explosive_etf_tracker(data)
