    ...
    try:
        ...
        if out.get('sbp_reserves') is None:
            ...
            if _res and _res.get('sbp_bn') is not None:
                ...
                _lg = safe_get(EXISTING, 'macros', 'psx', 'sbp_reserves')
                log(f'  ✓ SBP reserves (ecodata, live): SBP {_res.get("sbp_bn")}bn / bank {_res.get("bank_bn")}bn / total {_res.get("total_bn")}bn as on {_res.get("as_of")} (prior {_lg})')
        if out.get('sbp_reserves') is None:
