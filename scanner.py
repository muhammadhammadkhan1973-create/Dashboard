...
# never raises; touches NO data/screening/scoring/IM3/TCE/the frozen ledger → respects the Sept freeze.
TV_ISIN_FILTER_PROBE = True   # PRE-ARMED for ONE run; read [TV isin-filter probe] block, then lock or pivot
...
def screen_psx_universe():
    ...
    try:
        ...
        probe_etf_isin_feeds()              # World ETF Engine Phase 6 probe — gated off, verdict locked (nginx 403 dead end)
        probe_tv_isin_column()              # World ETF Engine Phase 6b probe — gated off, verdict locked (isin column CONFIRMED)
        probe_tv_isin_filter()              # World ETF Engine Phase 6c probe (logging-only): isin as a FILTER on European market scans
    except Exception as _e:
...
def build_etf_recommendations(country_rs, sector_booming, global_theme):
    """Phase 3: World ETF Engine — joins the three signal sources (country RS, US sector
    booming, global theme) to buyable ISINs via resolve_etf_for_signal(). Produces a compact
    list the dashboard can render directly: top country pick, top US sector pick, top global
    theme pick — each with the resolved fund + signal context that fired it.
    ...
    try:
        ...
        # 2. Top US sector (Booming/Favoured)
        eligible_sectors = [r for r in (sector_booming or [])
                             if r.get('band') in ('Booming', 'Favoured')]
        if eligible_sectors:
            top_s = max(eligible_sectors, key=lambda r: r.get('score') or 0)
        ...
        # 3. Top global theme (Booming/Favoured)
        eligible_themes = [r for r in (global_theme or []) if r.get('band') in ('Booming', 'Favoured')]
        if eligible_themes:
            top_t = max(eligible_themes, key=lambda r: r.get('score') or 0)
            # Theme -> best-fit category via its top country's ETF category
            top_country_cat = _COUNTRY_ETF_CAT.get(top_t.get('top_country'))
            if top_country_cat:
                fund = resolve_etf_for_signal(top_country_cat, fallback_keys=['Equity Asia Pacific', 'Equity World'])
                if fund:
                    recs.append({
                        'door': 'global_theme', 'signal': top_t['theme'],
                        'reason': f"score {top_t.get('score','—')} · top {top_t.get('top_country','—')} "
                                  f"{top_t.get('top_country_ret6m','—')}%",
                        'fund': fund,
                    })
        # v1.126.1: merge picks that resolve to the SAME ISIN (e.g. a country also
        # being the top country of a booming theme is a genuine double-confirmation,
        # not a duplicate) — combine doors/reasons into one entry, never show the
        # same fund twice as if it were two independent ideas.
        _by_isin = {}
        for r in recs:
            isin = r['fund']['isin']
            if isin in _by_isin:
