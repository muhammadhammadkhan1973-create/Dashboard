...
_PSX_YAHOO_SKIP = {'GAL', 'QTECH'}  # expand if other PSX names go stale on Yahoo
...
def fetch_world_lei():
    """v1.123.0: REPLACED FRED OECD CLI with theglobaleconomy.com LEI scraper.
    FRED OECD CLI series had 18-month lag (as_of 2024-01, stale 2022-11 for Euro/CH).
    TGE leading-economic-index page: same site + pattern as Pakistan CPI _tge() already
    proven on the runner. 13 countries: original 9 + Korea, Taiwan, NZ, Switzerland restored.
    Expansion = MoM score > 0 (positive monthly change); contraction = <= 0.
    Probe: logs raw val+date per country so runner confirms parse on first run.
    Guarded per-country; a miss omits that entry; full crash returns {}.
    DATA-only; never touches screen / scoring / TCE / frozen ledger."""
    # TGE slug → display name. Slug = the URL path segment on theglobaleconomy.com.
    # Probe note: if a country returns val=None, check the runner log for the raw seg snippet
    # and adjust the slug or fall back to an alternative indicator.
    _TGE_COUNTRIES = [
        ('Japan',        'Japan'),
        ('India',        'India'),
        ('USA',          'United-States'),
    ...
    out = {}
    ok = 0
    for display, slug in _TGE_COUNTRIES:
        try:
            val, date_str = _tge_lei(slug, 'leading-economic-index', diag=True)
            if val is None:
                # Fallback: try business-confidence for countries where LEI page may differ
                val2, date2 = _tge_lei(slug, 'business-confidence', diag=False)
                if val2 is not None:
                    val, date_str = val2, date2
                    log(f'  [World LEI] {display}: LEI page empty, using business-confidence {val} ({date_str})')
            if val is None:
                log(f'  [World LEI] {display}: no data from TGE — skipping')
                continue
            # Parse date string → YYYY-MM
            as_of = None
            if date_str:
                import re as _re
                dm = _re.search(r'(\d{4}).*?(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)|'
                                r'(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec).*?(\d{4})', date_str)
                if dm:
                    if dm.group(1):
                        yr, mn = int(dm.group(1)), _MMAP.get(dm.group(2), 1)
                    else:
                        mn, yr = _MMAP.get(dm.group(3), 1), int(dm.group(4))
                    as_of = f'{yr}-{mn:02d}-01'
            # TGE LEI is a monthly index score (not 100-centred like OECD CLI).
            # Signal: positive value = above trend (expansion); negative = below (contraction).
            # Slope/trend: val itself IS the MoM change score.
            signal = 'expansion' if val > 0 else 'contraction'
            trend  = 'rising'   if val > 0 else ('falling' if val < 0 else 'flat')
            out[display] = {
                'lei':      round(val, 3),
                'slope':    round(val, 3),   # TGE score IS the slope/MoM change
                'trend':    trend,
                'momentum': round(val, 3),
                'signal':   signal,
                'as_of':    as_of or date_str or 'n/a',
            }
            ok += 1
        except Exception as _e:
            log(f'  [World LEI] {display}: crash {type(_e).__name__}: {str(_e)[:50]}')
    log(f'  [World LEI] {ok}/{len(_TGE_COUNTRIES)} countries fetched (TGE)')
    return out
...
def fetch_country_rs():
    ...
    for country, idx_sym, fx_sym in _COUNTRIES:
        try:
            ...
            # Signal: hedge if deeply negative, overweight if strongly positive
            if ret6 is not None:
                signal = 'Overweight' if ret6 > 15 else (
                         'Hedge'      if ret6 < -10 else (
                         'Neutral'    if -5 <= ret6 <= 5 else 'Avoid'))
            else:
                signal = 'Neutral'
    ...
    log(f'  [Country RS] {len(results)}/9 countries ranked')
    return results
