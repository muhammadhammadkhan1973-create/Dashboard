...
def main():
    ...
    try:
        # v1.222.0: Business Recorder LIVE total first (probe-confirmed fresh, age 0d); frozen SCS PDF fallback.
        _mts = {}
        try:
            _mts = fetch_brec_mts()
        except Exception as _be:
            warn(f'brec MTS failed (non-blocking, will try SCS): {_be}')
        if not (_mts.get('market') or {}).get('total_mn'):
            _mts = fetch_psx_mts()
