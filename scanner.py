        ...
        for market in ['uk', 'germany']:
            ...
            try:
                ...
                if rec.get('isin') != isin:
                    log(f'    {label} [{market}]: row isin mismatch, skip')
                    continue
                perf_vals = {c: rec.get(c) for c in CAND_COLS}
                nonnull = {k: v for k, v in perf_vals.items() if v is not None}
                log(f'    \u2713 {label} [{market}]: sym={rows[0].get("s","")} close={rec.get("close")} -> {perf_vals}')
                if nonnull:
                    n_hit += 1
                found = True
            except Exception as e:
