...
# ============================ Momentum-40 Watchlist ============================
# Non-leveraged ETFs with YTD >= 40% from ETF_Universe_8.xlsx (as of data collection date).
# Owner-confirmed: include the 1 crypto product (21shares Hyperliquid ETP).
# Leveraged/short products (KOR3, SOX4, 3LNV etc.) EXCLUDED — daily-reset, not buy-and-hold.
# Ranked by YTD descending. Displayed independently on the World ETF tab — never confused
# with the engine's own recommendations (different badge: "Momentum-40").
_MOMENTUM_40 = [
...
def build_etf_recommendations(country_rs, sector_booming, global_theme):
    ...
    v1.131.0 multi-pick + Korea/Taiwan two-door rule:
      Door 1 (country): top 3 eligible countries by 6M USD return (Overweight/Hedge signal)
      Door 2 (sector):  top 2 US sectors (Booming first, then Favoured)
      Door 3 (theme):   top 2 booming themes x top 2 countries per theme (Asia-Tech fires BOTH Korea + Taiwan)
    Same-ISIN picks from multiple doors merge into a single double-confirmed entry.
    ...
    # Korea/Taiwan two-door: when a theme fires, surface these countries in priority order
    ...
    try:
        ...
        # Door 3: top 2 booming themes x top 2 countries per theme (Korea/Taiwan two-door rule)
            ...
            for country_name in ordered:
                ...
                if cat:
                    ...
                    if fund:
                        ...
                        n_theme += 1
        # Merge identical ISINs - same fund via multiple doors = genuine double-confirmation
        _by_isin = {}
        for r in recs:
            isin = r['fund']['isin']
            if isin in _by_isin:
...
def main():
    ...
    data['etf_momentum_40'] = _MOMENTUM_40   # World ETF Engine: Momentum-40 watchlist (34 non-leveraged ETFs up >=40% YTD)
    ...
    # Lightweight: only 1-3 recommendations, 1-2 TV scan requests each.
    _n_priced = 0
    for _rec in (data.get('etf_recommendations') or []):
        ...
        try:
            ...
            pass
    if _n_priced:
        log(f'  [ETF live prices] {_n_priced} recommendation(s) priced via isin-filter resolver')
