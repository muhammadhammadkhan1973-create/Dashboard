...
A1_RATIO_MIN   = 1.5    # A1 OPNP NP/OP growth ratio
...
_FIN_SECTORS = {'financial services', 'financials', 'finance', 'bank', 'banks', 'banking'}

def _looks_like_bank(sector, name):
    s = (sector or '').strip().lower(); n = (name or '').strip().lower()
    if s in _FIN_SECTORS: return True
    if 'bank' in s or 'financ' in s: return True
    if 'bank' in n: return True
    return False
...
def classify_explosive(cond):
    ...
    if growth and accel and cash_fail:       return 'QUALITY-GROWTH (growth + accel, cash unconfirmed)'
        ...
        try:
            ...
            if market == 'us' and yf is not None:
                try:
                    ...
                    pass
                time.sleep(YF_DELAY)
            # Fallback / PSX: embedded growth fields
            if rec is None:
                rec = score_explosive_candidate(c)
            # Bank/financial carve-out (BOTH markets): OP/NP/CFO conditions are invalid
            # for banks (NII != revenue, provisioning distorts NP, deposit flows distort
            # CFO). Route them to the bank model (IM3 System B), flagged out of the screen.
            if rec is not None:
                isfin = _looks_like_bank(rec.get('sector'), rec.get('name'))
                rec['is_financial'] = isfin
                if isfin:
...
def main():
    ...
    _us_analyst = fetch_us_analyst_block([c.get('ticker') for c in _us_pool])
    for _c in _us_pool:
