...
def fetch_justetf_holdings(isin, top=6):
        ...
        for h in holdings:
            ...
            try:
                if src == 'ishares':
                    ...
                    _nm = fetch_justetf_holdings(_isin, top=10) if _isin else None
                    # v1.215.16: suffix-aware split -- justETF names like 'Taiwan Semiconductor
                    # Manufacturing Co., Ltd' must not shed 'Ltd'/'Inc' into standalone rows
                    # (same re-merge rule as the index holdChips renderer).
                    _parts = []
                    _sfx = re.compile(r'^(Inc\.?|Ltd\.?|LLC|L\.?P\.?|Corp\.?|Co\.?|plc|PLC|S\.?A\.?|N\.?V\.?|AG|SE|SpA|Oyj|AB|ASA|KGaA|Bhd)( [A-Z][A-Za-z]{2,15})?$', re.I)  # v1.215.17: optional trailing country word -- AMEC's legal name is '..., Inc. China'
                    for _n in (_nm.split(',') if _nm else []):
                        _n = _n.strip()
                        if not _n:
                            continue
                        if _parts and _sfx.match(_n):
                            ...
                            _parts.append(_n)
                    rows = [{'ticker': None, 'name': _n, 'weight': None} for _n in _parts]
                else:
