...
def build_m2_watch(data, existing):
    try:
        ...
        # v1.207.1: the speculative (turnaround) lane structurally maxes ~2 signals (small caps
        # have no Zacks coverage and sit outside the TCE/fund pools), so a pure signal-count
        # top-40 cut squeezed out EVERY actual turnaround -- the sleeve's namesake was invisible.
        # Guarantee the lane a seat: every flagged speculative name (capped 12) is always shown
        # alongside the top disciplined names; merged list re-sorted for display.
        # v1.207.2: the D-127 reference names (CRDO, GEV) are the engine's tuning forks --
        # they must ALWAYS be visible rows on the tab when flagged, never just a PASS/FAIL
        # count (the v1.207.1 seat re-cut dropped GEV, a 2-signal name, off the display).
        _CAL = ('CRDO', 'GEV')
        _cal = [w for w in watch if w['ticker'] in _CAL]
        for w in _cal:
            w['ref'] = True
        _spec = [w for w in watch if w.get('lane') == 'speculative' and w['ticker'] not in _CAL][:12]
        _disc = [w for w in watch if w.get('lane') != 'speculative' and w['ticker'] not in _CAL][:40 - len(_spec) - len(_cal)]
        shown = _cal + _disc + _spec
        shown.sort(key=lambda w: (-w['n_signals'], -(w['keystone'] or 0)))
        calib = {t: any(w['ticker'] == t for w in watch) for t in ('CRDO', 'GEV')}
        data['m2_watch'] = {'as_of': dt.date.today().isoformat(), 'n_scanned': len(pool),
                            'n_flagged': len(watch), 'min_signals': 2, 'watch': shown,
                            'calibration': calib}
        log(f"  [M2 watch] scanned={len(pool)} flagged={len(watch)} (>=2 leading signals; shown {len(shown)}: {len(_disc)} disciplined + {len(_spec)} turnaround)"
