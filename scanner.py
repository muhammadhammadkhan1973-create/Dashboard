...
# v1.179.0 FOUNDATION UNIVERSE (from TradingView): the shared live whole-market scan the M2 build
# plan names as the 'universe keystone'. FOUNDATION_UNIVERSE_CAP = how many US common names the
# broad america/scan returns (mcap-desc, >=$300M, no ceiling). FOUNDATION_EXPLOSIVE_ADD = how many
# accelerating LARGE-CAPS (> the small-cap ceiling, not already in the pool) get added to the
# Explosive screen -> closes the coverage gap (Corning/Bloom/Marvell/Comfort Systems were invisible
# because the large-cap universe was a frozen 218-name hand-list).
FOUNDATION_UNIVERSE_CAP  = 2400
FOUNDATION_EXPLOSIVE_ADD = 120
...
def fetch_foundation_universe():
    """v1.179.0 — FOUNDATION UNIVERSE (from TradingView). ONE broad america/scan across the WHOLE
    US common-stock market (mcap >= $300M, NO upper ceiling), sorted mcap-desc, capped at
    FOUNDATION_UNIVERSE_CAP. This is the shared live universe the M2 build plan calls the 'universe
    keystone'; its first consumer is the Explosive screen, which previously only saw the small-cap
    band + a FROZEN 218-name curated large-cap list -> accelerating large-caps (Corning / Bloom /
    Marvell / Comfort Systems ...) were never scanned. Reuses the proven _US_TV_COLS POST (same
    endpoint/columns as the small-cap prefilter, already validated on the runner). Builds records in
    the SAME shape screen candidates use, WITHOUT the band/curated-list DROP (that gate is exactly
    what hid the large-caps). Returns a list of records (all sizes); [] on any failure so the caller
    degrades to prior behaviour (Explosive pool == survivors, no change)."""
    def _bare(s):
        return s.split(':')[-1]
...
def main():
    ...
    try:
        ...
        if _foundation:
            ...
            _lg = sum(1 for r in _foundation if (r.get('market_cap') or 0) > US_SMALL_CAP_MAX)
            log(f'  [Foundation Universe] TradingView america scan: {len(_foundation)} common US names '
                f'({_lg} large-cap > ${US_SMALL_CAP_MAX/1e9:.0f}bn); added top {len(_adds)} accelerating '
                f'large-caps to Explosive pool ({len(us_all_survivors)} -> {len(us_explosive_pool)})')
        else:
