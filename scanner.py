...
TV_ISIN_COLUMN_PROBE = True   # PRE-ARMED for ONE run; read the [TV isin-column probe] block, then lock or flip False
...
def screen_psx_universe():
    ...
    try:
        ...
        probe_etf_isin_feeds()              # World ETF Engine Phase 6 probe (logging-only): ISIN -> TV price + stockanalysis holdings (NO Yahoo/FMP) -- gated off, verdict locked (dead end)
        probe_tv_isin_column()              # World ETF Engine Phase 6b probe (logging-only): does the proven america/scan endpoint expose an 'isin' column?
    except Exception as _e:
