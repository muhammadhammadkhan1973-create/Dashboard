...
def _sec_im3_statements(ticker):
    ...
    inc_df = frame({'Total Revenue': rev, 'Operating Income': opi, 'Net Income': ni,
                    ...
                    'EBITDA': ebitda, 'Interest Expense': ie})
    ...
    cf_df  = frame({'Operating Cash Flow': ocf, 'Capital Expenditure': {y: -v for y, v in cap.items() if v is not None},
                    'Free Cash Flow': fcf, 'Depreciation': dep})
...
# names face the SAME deep-score gate -- evidence widens the pool, it never lowers the bar.
M1_OV_MOM_PTS   = 3.0
M1_OV_EPS_PTS   = 5.0
...
def build_m1_buylist(data, existing):
        ...
        if len(_all6) >= 200:                      # need a real market baseline (perf feed live)
            ...
            for _o in overrides:
                ...
                log(f"  [M1 override] {_o['sector']} JOINS the pool ({_o['gates']}/3 gates): "
                    ...
                    f"-- off-regime, same 61% deep-score gate")
    except Exception as _e:
    ...
    buys = eligible[:8]
    # audit-trail hygiene: carry prior audit rows for TTL-carried scores, prune names no longer pooled
