...
def update_tce_predictions(prev, today_iso, rows,
    ...
    # PSX entries logged before the L3 TV-price cutover used the STALE dps.psx EOD basis (confirmed:
    # TRG 164 vs real 72, KPUS 49 vs real 2443) — their forward returns/alpha would be garbage at
    # maturity. Reset each OPEN PSX pick's entry price/date/benchmark to the correct current basis
    # ONCE, then flag done so it never repeats. US entries (always-correct Yahoo) are untouched.
    rebaselined = 0
    if not prev.get('psx_rebaselined_v136'):
        _cb = bench.get('psx')
        for p in preds:
