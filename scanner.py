...
def fetch_justetf_holdings(isin, top=6):
    ...
    issuer's published holdings (VanEck / HSBC / Amundi / Xtrackers / Franklin / Global X / KraneShares, not
    just iShares), keyed by ISIN. Returns 'Name1, Name2, ...' or None. Anchors on the Holdings section so it
    never mistakes the country/sector %-tables for constituents. The first 3 calls log a [justETF diag] line
    (HTTP status, body size, and the raw markup around the holdings region) so a zero result is debuggable --
    reachability vs parse-structure -- instead of silently returning None."""
    ...
    try:
        ...
        if diag:
            _JUSTETF_DIAG['n'] += 1
            low0 = html.lower()
            # show raw context around the first plausible holdings anchor so the real structure is visible
            anchor = -1
            for probe in ('largest positions', 'top 10 holdings', 'holdings', 'instrument'):
                anchor = low0.find(probe)
                if anchor > 0:
                    break
            ctx = html[max(0, anchor - 120):anchor + 520].replace('\n', ' ') if anchor > 0 else '(no holdings anchor found)'
            ctx = re.sub(r'\s+', ' ', ctx)
            log(f'    · [justETF diag] {isin} HTTP {r.status_code} {len(html)}B anchor@{anchor}: {ctx[:560]}')
        ...
        if i < 0:
            return None
        ends = [x for x in (low.find('countr', i + 8), low.find('sector', i + 8), i + 7000) if x > i]
        block = html[i:min(ends)] if ends else html[i:i + 7000]
        text = re.sub(r'<[^>]+>', '\n', block).replace('&amp;', '&').replace('&#39;', "'")
        names, seen = [], set()
        _stop = ('holding', 'top ', 'show', 'name', 'weight', 'as of', 'countr', 'sector',
                 'this etf', 'the etf', 'more', 'data ', 'source')
        for m in re.finditer(r'([A-Z][A-Za-z0-9&.,\'/\-() ]{2,45}?)\s*\n+\s*(\d{1,2}(?:\.\d{1,2})?)\s*%', text):
            nm = m.group(1).strip(' .,-\n')
            nl = nm.lower()
            if not (2 <= len(nm) <= 45) or any(nl.startswith(s) for s in _stop) or nm in seen:
                continue
            seen.add(nm)
            names.append(nm)
            if len(names) >= top:
