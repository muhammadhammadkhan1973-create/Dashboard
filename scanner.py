...
def fetch_world_lei():
    """Phase 0-A: Pull 9-country OECD Leading Economic Indicator (LEI) series from FRED
    via fredapi (same Fred(api_key=FRED_KEY) pattern as the existing _fred_series calls).
    Returns {country: {slope, trend, momentum, signal, as_of}} for:
    Australia, Canada, China, Euro-area, Japan, New-Zealand, Switzerland, UK, USA.
    Guarded — a country miss omits that entry; a full crash returns {}.
    ...
    _LEI_SERIES = {
        ...
        'Euro-area':   'EA19LOLITONOSTSAM',
        'Japan':       'JPNLOLITONOSTSAM',
        # New-Zealand: removed v1.119.0 — FRED carries no OECD CLI series for NZL (neither NZLLOLITONOSTSAM nor OECDNZLCLIAMP exist); honest 8/9
    ...
    try:
        ...
        for country, series_id in _LEI_SERIES.items():
            try:
                ...
                latest   = vals[-1]
                prev1    = vals[-2]
                prev3    = vals[-4] if len(vals) >= 4 else vals[0]
                prev6    = vals[-7] if len(vals) >= 7 else vals[0]
                avg3     = sum(vals[-3:]) / 3
                ...
                log(f'  [World LEI] {country} skip: {type(_e).__name__}: {str(_e)[:40]}')
        log(f'  [World LEI] {len(out)}/9 countries fetched')
    except Exception as e:
...
def fetch_country_rs():
    """Phase 0-C: Pull 9-country equity-index + FX monthly closes via Yahoo Finance and
    compute 3/6/12-month USD-adjusted price momentum + trend label + FX contribution.
    ...
    _USD_PER_UNIT = {'EURUSD=X', 'GBPUSD=X', 'AUDUSD=X'}
...
def compute_sector_booming(zacks_sectors, cot_futures):
    ...
    # Extract COT signals for mapped sectors
    cot = cot_futures or {}
    cot_by_sector = {}
    # Energy -> Crude, Utilities -> NatGas, Staples -> Corn/Agriculture
    for sec, contract_key in [('Energy','Crude'), ('Utilities','NatGas'),
