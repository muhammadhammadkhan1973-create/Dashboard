...
COUNTRYECONOMY_PROBE = True  # v1.185.0 one-shot runner probe of countryeconomy.com; flip False after reading the [countryeconomy probe] log lines


def probe_countryeconomy():
    """v1.185.0 -- ONE-SHOT reachability + static-HTML probe of countryeconomy.com on the GitHub runner.
    LOGGING-ONLY and freeze-safe: touches NO data / screening / scoring / IM3 / TCE / index. It answers two
    questions before we build anything on the site: (1) can the runner reach it at all (datacenter IPs are
    sometimes blocked even when a browser can), and (2) do the macro figures sit in the RAW HTML (static,
    parseable) or behind JavaScript (a dead end, the way theglobaleconomy's multi-country pages were)? If
    both hold, its genuinely useful fields -- country risk premium, sovereign ratings, and Pakistan's
    deficit/debt (on manual entry today) -- can later feed the World Economy + Pakistan tabs and M1's
    country layer. Never wired into a macro until this confirms. Flip COUNTRYECONOMY_PROBE False after the read."""
    import re as _re
    pages = [('US', 'https://countryeconomy.com/countries/usa'),
             ('Pakistan', 'https://countryeconomy.com/countries/pakistan')]
    LABELS = ['Risk premium', 'Rating', 'Deficit', 'National debt', 'GDP', 'CPI', 'Unemployment']
    for tag, url in pages:
        try:
            r = requests.get(url, headers={'User-Agent': UA}, timeout=15)
            body = r.text or ''
            low = body.lower()
            labels_hit = [k for k in LABELS if k.lower() in low]
            has_dollar = bool(_re.search(r'\$[\d,]{4,}', body))     # e.g. $30,769,700M in the raw HTML
            has_pct = bool(_re.search(r'-?\d+\.\d+\s*%', body))     # e.g. 2.2%
            has_rating = bool(_re.search(r'\b(AAA|AA[+-]?|A[+-]?|BBB[+-]?|BB[+-]?|B[+-]?|CCC[+-]?)\b', body))
            log(f'  [countryeconomy probe {tag}] HTTP {r.status_code} · {len(body)} bytes · '
                f'labels {len(labels_hit)}/{len(LABELS)} ({",".join(labels_hit) or "none"}) · '
                f'raw-numbers: $={has_dollar} %={has_pct} rating={has_rating}  '
                f'-> {"STATIC/fetchable" if (has_dollar or has_pct) and len(labels_hit) >= 3 else "check (JS/blocked?)"}')
        except Exception as e:
            log(f'  [countryeconomy probe {tag}] FAILED: {e.__class__.__name__}: {str(e)[:120]}')
...
def main():
    ...
    # v1.185.0: one-shot countryeconomy.com reachability/static-HTML probe (logging-only, freeze-safe).
    if COUNTRYECONOMY_PROBE:
        try:
            probe_countryeconomy()
        except Exception as e:
            log(f'  [countryeconomy probe] wrapper failed: {e}')
