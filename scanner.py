...
def fetch_justetf_holdings(isin, top=6):
    ...
    just iShares), keyed by ISIN -- the probe confirmed the profile returns HTTP 200 with the holdings in the
    page. Returns 'Name1, Name2, ...' or None. Anchors on the Holdings section (between the 'Holdings' heading
    and the Countries/Sectors tables) so it never mistakes the country/sector %-tables for constituents. Never
    raises; a miss returns None (caller leaves '\u2014'). Synthetic/swap ETFs legitimately have no holdings -> None."""
    try:
        ...
        if i < 0:
            return None
        # bound the block at the next section so country/sector %-tables can't leak in
        ...
        for m in re.finditer(r'([A-Z][A-Za-z0-9&.,\'/\-() ]{2,45}?)\s*\n+\s*(\d{1,2}(?:\.\d{1,2})?)\s*%', text):
            ...
            if not (2 <= len(nm) <= 45):
                continue
            if any(nl.startswith(s) for s in _stop):
                continue
            if nm in seen:
                continue
            seen.add(nm)
