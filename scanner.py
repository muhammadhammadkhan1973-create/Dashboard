...
# columns are locked NEXT increment from the first-run RAW dump (a heavy date-watermark blocked a remote read
# of the table). FAILS SAFE: empty -> last-good. NEVER touches screening/scoring/TCE -> respects the Sept freeze.
SCS_MTS_INGEST = True
MTS_DUMP_RAW = True   # one-shot: log the runner's real MTS text to lock the per-symbol columns; flip False after reading
...
_MTS_MARKET_LABELS = [
    (('mts investment', 'total mts', 'total investment', 'total financing', 'total leverage', 'gross financing'), 'total_investment_mn'),
    (('net investment', 'net mts', 'change in investment', 'net financing'), 'net_investment_mn'),
    (('total volume', 'mts volume', 'financed volume'), 'total_volume'),
    (('weighted average', 'wtd. avg', 'wtd avg', 'average rate', 'mts rate', 'mark-up rate', 'markup rate'), 'wavg_rate'),
    (('number of scrips', 'no. of scrips', 'no of scrips', 'total scrips', 'active scrips', 'eligible securities'), 'scrips'),
]
...
def _parse_mts(text):
    """PURE. Walk the linearised PDF text (date-watermark noise = single-char lines, dropped by len<=3). Extract
    as_of, market totals (label-anchored, best-effort), the MTS book (symbols carrying positions), per-symbol raw
    numeric tokens, and a substantive-line diagnostic. Returns a dict (never raises)."""
    as_of = '?'
    for line in text.splitlines():
        m = re.search(r'([A-Z][a-z]+ \d{1,2},?\s*\d{4})', line)
        if m:
    ...
    for raw in text.splitlines():
        line = raw.strip()
        if len(line) <= 3:
            continue  # drop the repeating date-watermark single-char noise
        rowlike = bool(re.match(r'^[A-Z][A-Z0-9]{1,7}\s', line))
        if not rowlike:
            low = line.lower()
            nums = re.findall(r'-?[\d,]+\.?\d*', line)
            if nums:
                for labels, field in _MTS_MARKET_LABELS:
                    if field in market:
                        continue
                    if any(lab in low for lab in labels):
                        market[field] = _mts_num(nums[-1] if field == 'wavg_rate' else nums[0])
            if len(diag) < 60:
                diag.append(line)
            continue
        parts = line.split()
        ...
        if not re.fullmatch(r'[A-Z][A-Z0-9]{1,7}', sym):
            ...
            continue
        tail = parts[1:]
        nums = [_mts_num(x) for x in tail if _mts_num(x) is not None]
        nonnum = sum(1 for x in tail if _mts_num(x) is None)
        # a real per-symbol MTS row is SYM + numeric columns; reject prose/title lines (e.g. the
        # "MTS Daily Report February 6, 2026" header) where word tokens outnumber the numbers.
        if len(nums) < 2 or len(nums) < nonnum:
            if len(diag) < 60:
                diag.append(line)
            continue
        by_ticker[sym] = {'nums': nums}
        if sym not in book:
            book.append(sym)
        if len(diag) < 60:
            diag.append(line)
    return {'as_of': as_of, 'market': market, 'book': book, 'by_ticker': by_ticker, '_diag': diag}
...
def fetch_psx_mts():
    ...
    if MTS_DUMP_RAW:
        log('  [Wave PSX-R MTS] RAW table text (first 60 substantive lines; flip MTS_DUMP_RAW False once locked):')
        for ln in parsed.get('_diag', []):
            log('      | ' + ln[:160])
    parsed.pop('_diag', None)
    if not parsed.get('market') and not parsed.get('book'):
        log('  [Wave PSX-R MTS] 0 fields parsed -> empty (see RAW dump above; confirm labels/columns on the runner)')
        return {}
    log(f'  [Wave PSX-R MTS] as-of {parsed["as_of"]}: market={parsed["market"]}, {len(parsed["book"])} symbols in the MTS book')
    return parsed
