...
JUSTETF_HOLDINGS_PROBE = True   # v1.215.3: one-shot runner probe for a general UCITS-by-ISIN holdings source; flip False after reading [justETF probe]


def _attach_discovery_holdings(data):
    """Fill the live-discovery funds' 'holdings' from sources the dashboard ALREADY has -- ZERO new fetch.
    Each discovery ISIN is matched against the hydrogen / emerging-themes / metals watchlists, the
    recommendations, and the look-through funds -- all of which already carry real top-holdings. Discovery
    itself doesn't fetch underlyings (60 rotating funds) and there is no general UCITS-by-ISIN holdings
    source proven on the runner yet (stockanalysis 404s UCITS-only funds; the iShares issuer file covers
    only the pinned ISINs), so this reuse pass fills every discovery fund the dashboard already knows and
    leaves the rest honestly '-' (em-dash) until the justETF general source is wired. Returns the list of
    still-uncovered ISINs (richest-YTD first) for the probe."""
    ...
    def _hstr(h):
        ...
        if isinstance(h, list):
            return ', '.join(str(x) for x in h) if h else None
        return None
    ...
    for fund in ((data.get('live_lookthrough') or {}).get('funds') or []):
        iz = fund.get('isin')
        names = [(h.get('name') or h.get('ticker')) for h in (fund.get('holdings') or [])
                 if (h.get('name') or h.get('ticker'))]
        if iz and names:
            hmap.setdefault(iz, ', '.join(names[:6]))
    ...
    for d in disc:
        ...
        if hs:
            ...
            uncovered.append((d.get('ytd') if d.get('ytd') is not None else -1e9, iz, d.get('name')))
    uncovered.sort(reverse=True)   # richest-YTD uncovered first = what the owner sees at the top of the tab
    log(f'  [discovery holdings] {filled}/{len(disc)} filled from existing dashboard holdings (reuse, zero fetch); '
        f'{len(uncovered)} still unsourced (need general source)')
    return [u[1] for u in uncovered if u[1]]


def probe_justetf_holdings(isins):
    """One-shot logging-only runner probe (JUSTETF_HOLDINGS_PROBE) for a GENERAL UCITS-by-ISIN holdings source.
    justETF publishes top-holdings per fund keyed by ISIN; sandbox can't reach it, so this logs -- per candidate
    URL -- HTTP status, byte size, and whether holdings-like tokens (%, company names, a holdings table/JSON) sit
    in the RAW response, so the next version wires the correct parser (probe-first: past sources looked fine but
    failed on the runner -- TGE JS-rendered, FMP 403, iShares screener 500). Never raises; wrapped by the caller."""
    if not isins:
        log('  [justETF probe] no uncovered discovery ISINs to probe')
        return
    log(f'  [justETF probe] testing a general UCITS-by-ISIN holdings source on {min(len(isins),8)} uncovered discovery fund(s):')
    for iz in isins[:8]:
        for label, url in (
            ('profile', f'https://www.justetf.com/en/etf-profile.html?isin={iz}'),
            ('ajax-hold', f'https://www.justetf.com/api/etfs/{iz}/quote?locale=en&currency=USD'),
        ):
            try:
                r = requests.get(url, headers={'User-Agent': UA, 'Accept': 'text/html,application/json'}, timeout=15)
                body = r.text or ''
                low = body.lower()
                hint = []
                if 'holding' in low:
                    hint.append('has-"holding"')
                if '%' in body and any(t in low for t in ('inc', 'corp', 'ltd', 'plc', 'co.', 'ag', 'nv')):
                    hint.append('names+%')
                if low.lstrip().startswith('{') or low.lstrip().startswith('['):
                    hint.append('json')
                log(f'    · {iz} [{label}] HTTP {r.status_code} {len(body)}B {"/".join(hint) or "no-holdings-tokens"}')
            except Exception as e:
                log(f'    · {iz} [{label}] EXC {type(e).__name__}: {str(e)[:50]}')
...
def build_live_etf_discovery(min_aum_m=0.0, per_segment=40, top_n=60):
    ...
    for _i, _r in enumerate(ranked):
        _r['rank'] = _i + 1
    _amin = min(aums) if aums else None
    _amax = max(aums) if aums else None
    log(f'  [ETF discovery] {len(ranked)} genuine UCITS ETFs discovered live '
        f'(kept {kept} pre-dedup, dropped {dropped} leveraged/ETP; aum raw range {_amin}..{_amax} -- tune floor next run); '
...
def main():
    ...
    try:
        _uncovered_isins = _attach_discovery_holdings(data)
        if JUSTETF_HOLDINGS_PROBE:
            probe_justetf_holdings(_uncovered_isins)
    except Exception as _e:
