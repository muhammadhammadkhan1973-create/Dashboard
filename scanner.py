...
_SECTOR_MED_COLS = ['sector', 'price_earnings_ttm', 'operating_margin', 'net_margin', 'return_on_equity', 'close', 'SMA200', 'Perf.W', 'Perf.1M', 'Perf.3M', 'price_target_average', 'earnings_per_share_forecast_next_fq', 'earnings_per_share_fq']

DIV_YIELD_PROBE = True  # one-run armed: dump which TV dividend-yield column is accepted + its units, then lock + flip False
...
def fetch_sector_medians(sample=2000):
    ...
    try:
        ...
        for d in rows:
            ...
            sec, pe, opm, npm, roe, close, sma200, pw, p1m, p3m, tgt, fqn, fqc = vals
            if not sec:
                continue
        ...
        for sec, a in acc.items():
            ...
            if any(k in rec for k in ('pe', 'op_margin', 'net_margin', 'roe', 'breadth_200dma', 'perf_w', 'perf_1m', 'perf_3m', 'tgt_upside', 'fwd_eps_q_g')):
...
def fetch_psx_sector_medians(sample=600):
    ...
    try:
        cols = ['name', 'sector', 'price_earnings_ttm', 'operating_margin', 'net_margin', 'return_on_equity',
                ...
                'price_target_average', 'earnings_per_share_forecast_next_fq', 'earnings_per_share_fq']
        body = {'columns': cols, 'range': [0, sample],
                'sort': {'sortBy': 'market_cap_basic', 'sortOrder': 'desc'}, 'markets': ['pakistan']}
        r = requests.post('https://scanner.tradingview.com/pakistan/scan', json=body,
                          headers={'User-Agent': UA, 'Accept': 'application/json'}, timeout=15)
