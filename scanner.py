...
_SECTOR_MED_COLS = ['sector', 'price_earnings_ttm', 'operating_margin', 'net_margin', 'return_on_equity', 'close', 'SMA200', 'Perf.W', 'Perf.1M', 'Perf.3M']
...
def fetch_sector_medians(sample=2000):
    ...
    try:
        ...
        for d in rows:
            ...
            sec, pe, opm, npm, roe, close, sma200, pw, p1m, p3m = vals
            if not sec:
                continue
            a = acc.setdefault(sec, {'pe': [], 'op_margin': [], 'net_margin': [], 'roe': [], 'b_above': 0, 'b_tot': 0,
                                     'perf_w': [], 'perf_1m': [], 'perf_3m': []})
        ...
        for sec, a in acc.items():
            ...
            if any(k in rec for k in ('pe', 'op_margin', 'net_margin', 'roe', 'breadth_200dma', 'perf_w', 'perf_1m', 'perf_3m')):
    ...
    try:
        ...
        if out:
            ...
            log(f'  [PSX sector medians] {len(out)} sectors (e.g. ' +
                ', '.join(f"{s} PE~{v.get('pe')}/ROE~{v.get('roe')}/1M~{v.get('perf_1m')}%" for s, v in _ex) + ')')
            return out
