    ...
    for slug, amc in _FMR_SOURCES:
        ...
        try:
            ...
            for _pat in (r'as at ([A-Z][a-z]+\.? \d{1,2},? \d{4})',
                         ...
                         r'\b(\d{1,2}[-/][A-Z][a-z]{2}[-/]\d{2,4})\b',
                         r'Report[ \-]+([A-Z][a-z]+ \d{4})',
                         r'\b([A-Z][a-z]{2,9} \d{1,2},? \d{4})\b'):
                _mm = re.search(_pat, text)
                if _mm:
                    asof = _mm.group(1); break
            for tk, w, _ in hold:
                agg[tk].append(w)
            for raw, w in unm:
