...
def compute_psx_devaluation(psx):
    ...
    try:
        ...
        max_score = 7   # rupee(2) + reserves(2) + reer(2) + rate(1); CA not scored by default
...
# === Wave SX engine (sector composite) ===
# TradingView sector groupings -> GICS (same map the index uses; IT/Comm split-aware).
_TV_TO_GICS = {
...
_GICS_ORDER = ['Information Technology','Health Care','Financials','Energy','Materials',
               'Industrials','Consumer Staples','Consumer Discretionary',
               'Communication Services','Utilities','Real Estate']

# COT contract per GICS — ONLY where a liquid futures contract actually tracks the
# sector (SX-5 honest COT). Everything else -> None -> COT is dropped (0 weight),
# never the S&P proxy masquerading as a sector signal.
_GICS_COT = {
    'Energy':                 'Crude',
    ...
    # Materials uses metals COT (gold/copper) read from macros.metals, handled inline.
}

# Weights (sum 100 when every signal present; re-normalised over the signals a
# sector actually has, so a missing leg never silently penalises it).
_W = {'mom': 25, 'breadth': 20, 'forward': 20, 'regime': 15, 'rs': 10, 'cot': 10}

_BANDS = [(80, 'Booming'), (62, 'Favoured'), (45, 'Neutral'), (30, 'Lagging'), (0, 'Avoid')]


def _regime_us(macros_us):
    """Scanner-side regime read from fields the scanner already has: growth axis =
    2s10s curve sign + ISM, inflation = core PCE. Mirrors the index quadrant so the
    favored list is identical, but computed ONCE here as the single source of truth."""
    pce = macros_us.get('core_pce')
    s2 = macros_us.get('us_2y'); s10 = macros_us.get('us_10y')
    slope = macros_us.get('us_2s10s')
    if slope is None and s2 is not None and s10 is not None:
        slope = s10 - s2
    growth_pos = (slope is None) or (slope > 0)   # positive/normal curve => growth expanding
    infl_hot = (pce is not None and pce > 2.5)
    if growth_pos and not infl_hot:
        name, fav = 'GOLDILOCKS', ['Information Technology','Communication Services','Consumer Discretionary','Financials','Industrials','Materials']
    elif growth_pos and infl_hot:
        name, fav = 'REFLATION', ['Energy','Materials','Industrials','Financials','Consumer Discretionary']
    elif (not growth_pos) and not infl_hot:
        name, fav = 'DISINFLATION', ['Utilities','Consumer Staples','Health Care','Real Estate']
    else:
        name, fav = 'STAGFLATION', ['Energy','Consumer Staples','Utilities','Communication Services']
    return name, set(fav), pce, slope


def _agg_gics(sector_medians):
    """Roll the TV-grouping sector_medians up to GICS: name-weighted (by 'n')
    averages of pe / roe / perf_1m / perf_3m / tgt_upside / div_yield / breadth."""
    acc = {}
    for tv, d in (sector_medians or {}).items():
        if tv.startswith('_') or not isinstance(d, dict):
            continue
        g = _TV_TO_GICS.get(tv)
        if not g:
            continue
        n = d.get('n') or d.get('breadth_n') or 1
        a = acc.setdefault(g, {'n': 0})
        for k in ('pe', 'roe', 'perf_1m', 'perf_3m', 'tgt_upside', 'div_yield', 'breadth_200dma'):
            v = d.get(k)
            if v is not None:
                a.setdefault(k, 0.0); a.setdefault(k + '_n', 0)
                a[k] += v * n; a[k + '_n'] += n
        a['n'] += n
    out = {}
...
def _score_linear(v, lo, hi):
    """Map v in [lo,hi] -> [0,100], clamped. lo<hi."""
    if v is None:
        return None
    return _clamp((v - lo) / (hi - lo) * 100.0)


def compute_sector_scores(data, market='us'):
    """Returns a ranked list of per-sector composites. See module header."""
    macros = (data.get('macros') or {}).get('us' if market == 'us' else 'psx') or {}
    sm = data.get('sector_medians') if market == 'us' else data.get('psx_sector_medians')
    sb = data.get('sector_breadth') if market == 'us' else data.get('psx_sector_breadth')
    zs = data.get('zacks_sectors') or {}
    cot = data.get('cot_futures') or {}
    metals = (data.get('macros') or {}).get('metals') or {}

    regime, favored, pce, slope = _regime_us(macros) if market == 'us' else (None, set(), None, None)
    gics = _agg_gics(sm or {})

    # market-wide RS reference = average sector 1M move (a sector beating the
    # average is showing relative strength; a light proxy until Wave T deepens).
    moms = [g['perf_1m'] for g in gics.values() if g.get('perf_1m') is not None]
    mkt_mom = (sum(moms) / len(moms)) if moms else 0.0

    rows = []
    sectors = _GICS_ORDER if market == 'us' else sorted(gics.keys())
    for g in sectors:
        a = gics.get(g, {})
        signals = []

        # --- 1) PRICE MOMENTUM (1M blended with 3M monthly-equivalent) ------
        m1, m3 = a.get('perf_1m'), a.get('perf_3m')
        mom_v = None
        if m1 is not None and m3 is not None:
            mom_v = 0.6 * m1 + 0.4 * (m3 / 3.0)          # both ~monthly %
        elif m1 is not None:
            mom_v = m1
        elif m3 is not None:
            mom_v = m3 / 3.0
        s_mom = _score_linear(mom_v, -6, 8) if mom_v is not None else None
        if s_mom is not None:
            _mv = m1 if m1 is not None else (m3 / 3.0 if m3 is not None else 0)
            signals.append({'name': 'Price momentum', 'score': s_mom, 'weight': _W['mom'],
                            'val': f"{'+' if _mv >= 0 else ''}{_mv:.1f}%/1M",
                            'dir': 'for' if s_mom >= 55 else ('against' if s_mom <= 40 else 'neutral')})

        # --- 2) 200-DMA BREADTH --------------------------------------------
        brv = None
        if sb:
            # sb is keyed by TV grouping; take the best-covered TV child of this GICS
            kids = [(tv, d) for tv, d in sb.items() if _TV_TO_GICS.get(tv) == g and isinstance(d, dict)]
            if kids:
                tot = sum(d.get('n', 0) for _, d in kids) or 1
                brv = sum(d.get('pct', 0) * d.get('n', 0) for _, d in kids) / tot
        if brv is None:
            brv = a.get('breadth_200dma')
        s_br = _score_linear(brv, 30, 85) if brv is not None else None
        if s_br is not None:
            signals.append({'name': '200-day breadth', 'score': s_br, 'weight': _W['breadth'],
                            'val': f"{brv:.0f}% in uptrend",
                            'dir': 'for' if brv >= 60 else ('against' if brv < 40 else 'neutral')})

        # --- 3) FORWARD (analyst target upside + Zacks rotation) -----------
        tgt = a.get('tgt_upside')
        z = zs.get(g) or {}
        zchg = z.get('pct_top_chg'); ztop = z.get('pct_top')
        fwd_parts = []
        if tgt is not None:
            fwd_parts.append(_score_linear(tgt, 5, 35))
        if ztop is not None:
            # rotation momentum: base breadth + change kicker
            fwd_parts.append(_clamp(ztop * 2 + (zchg or 0) * 1.5))
        s_fwd = (sum(fwd_parts) / len(fwd_parts)) if fwd_parts else None
        if s_fwd is not None:
            v = (f"target +{tgt:.0f}%" if tgt is not None else "") + \
                (f" · Zacks {ztop:.0f}%{'↑' if (zchg or 0) > 0 else ('↓' if (zchg or 0) < 0 else '')}" if ztop is not None else "")
            signals.append({'name': 'Forward (targets + Zacks)', 'score': s_fwd, 'weight': _W['forward'],
                            'val': v.strip(' ·'),
                            'dir': 'for' if s_fwd >= 55 else ('against' if s_fwd <= 40 else 'neutral')})

        # --- 4) REGIME FIT --------------------------------------------------
        if market == 'us':
            regf = g in favored
            s_reg = 75.0 if regf else 35.0
            signals.append({'name': 'Regime fit', 'score': s_reg, 'weight': _W['regime'],
                            'val': (regime + (' favored' if regf else ' not favored')),
                            'dir': 'for' if regf else 'neutral'})

        # --- 5) RELATIVE STRENGTH vs market (1M proxy) ---------------------
        if m1 is not None and moms:
            rel = m1 - mkt_mom
            s_rs = _score_linear(rel, -5, 5)
            signals.append({'name': 'RS vs market', 'score': s_rs, 'weight': _W['rs'],
                            'val': f"{'+' if rel>=0 else ''}{rel:.1f}pp vs avg",
                            'dir': 'for' if rel > 1 else ('against' if rel < -1 else 'neutral')})

        # --- 6) COT — honest: only where a real sector contract exists ------
        cot_key = _GICS_COT.get(g)
        cot_v = None; cot_label = None
        if g == 'Materials':
            # metals managed-money net read (gold + copper) already in macros.metals
            gp = metals.get('cot_gold_pct'); cp = metals.get('cot_copper_pct')
            parts = [x for x in (gp, cp) if x is not None]
            if parts:
                cot_v = sum(parts) / len(parts); cot_label = f"metals net {cot_v:+.0f}% OI"
        elif cot_key and cot_key in cot and isinstance(cot[cot_key], dict):
            poi = cot[cot_key].get('pct_oi')
            if poi is not None:
                cot_v = poi; cot_label = f"{cot_key} net {poi:+.0f}% OI"
        if cot_v is not None:
            s_cot = _score_linear(cot_v, -12, 12)
            signals.append({'name': 'COT positioning', 'score': s_cot, 'weight': _W['cot'],
                            'val': cot_label,
                            'dir': 'for' if cot_v > 2 else ('against' if cot_v < -2 else 'neutral')})

        # --- COMPOSITE ------------------------------------------------------
        if not signals:
            rows.append({'sector': g, 'score': None, 'band': 'n/a', 'signals': [],
                         'for_count': 0, 'against_count': 0, 'decisive': None,
                         'regime_favored': (g in favored)})
            continue
        wsum = sum(s['weight'] for s in signals)
        score = sum(s['score'] * s['weight'] for s in signals) / wsum
        for_c = sum(1 for s in signals if s['dir'] == 'for')
        against_c = sum(1 for s in signals if s['dir'] == 'against')
        # decisive = the signal that moves the composite furthest from neutral(50), weighted
        decisive = max(signals, key=lambda s: abs(s['score'] - 50) * s['weight'])
        band = next(b for thr, b in _BANDS if score >= thr)
        rows.append({'sector': g, 'score': round(score, 1), 'band': band,
                     'for_count': for_c, 'against_count': against_c,
                     'decisive': decisive['name'], 'regime_favored': (g in favored),
                     'signals': [{'name': s['name'], 'dir': s['dir'],
                                  'score': round(s['score'], 0), 'weight': s['weight'],
                                  'val': s['val']} for s in signals]})
    ...
    for i, r in enumerate(scored, 1):
        r['rank'] = i
    n = len(scored)
    return {'regime': regime, 'as_of': None, 'n': n,
            'sectors': scored + [r for r in rows if r['score'] is None]}
...
def main():
    ...
    # Wave PK-D: PSX PKR-devaluation alert (display/data-only multi-signal basket).
    try:
        data['psx_devaluation'] = compute_psx_devaluation((data.get('macros') or {}).get('psx') or {})
    ...
    # Wave SX1-5: transparent booming-sector composite (band+tally+rank), both markets.
    try:
        data['sector_scores_us'] = compute_sector_scores(data, 'us')
        _sxu = data['sector_scores_us']
        _top = [f"{r['sector']} {r['score']}({r['band']})" for r in _sxu.get('sectors', [])[:3] if r.get('score') is not None]
        log(f"[Wave SX] regime {_sxu.get('regime')} | {_sxu.get('n')} US sectors ranked | top: " + ' · '.join(_top))
    except Exception as e:
        log(f'[Wave SX] US sector scores crashed: {e}')
        data['sector_scores_us'] = EXISTING.get('sector_scores_us', {}) or {}
    try:
        data['sector_scores_psx'] = compute_sector_scores(data, 'psx')
    except Exception as e:
        log(f'[Wave SX] PSX sector scores crashed: {e}')
        data['sector_scores_psx'] = EXISTING.get('sector_scores_psx', {}) or {}
