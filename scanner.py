    ...
    for _mkt in _markets:
        try:
            ...
            for dd in rows:
                ...
                if not _etf_is_genuine_ucits(rec.get('name'), rec.get('typespecs'), rec.get('expense_ratio')):
                    dropped += 1; continue
                isin = rec.get('isin')
                if not isin:
                    continue
                ...
                cand = {'name': rec.get('name'), 'isin': isin,
                        ...
                        'category': _etf_category(rec.get('name')), 'live_ytd': round(ytd, 1) if ytd is not None else None,
