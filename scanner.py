        ...
        for _key in _grp:
            for _r in EXISTING.get(_key, []):
                if isinstance(_r, dict) and _r.get('im3') is not None and _r.get('ticker'):
                    ...
                    if isinstance(_r['im3'], dict) and _r['im3'].get('scan_rev') != IM3_SCAN_REV:
                        _dropped_stale += 1
                        continue  # old-scale grade (pre strength-only rework): drop, never display wrong-scale numbers
